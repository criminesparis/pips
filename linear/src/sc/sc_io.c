/*
  ...
  $Id: sc_io.c 1669 2019-06-26 17:24:57Z coelho $
	       ...
	       get_variable_name_t nom_var)
{
	...
	for (peq = ps->inegalites, count=0; peq!=NULL; peq=peq->succ, count++)
    inegalite_fprint(fp,peq,nom_var);

	assert(count==ps->nb_ineq);

	for (peq = ps->egalites, count=0; peq!=NULL; peq=peq->succ, count++)
    egalite_fprint(fp,peq,nom_var);
...
{
  FILE * f;
  char fn[512], *filename;
  int d;

  filename = "_sc.out";
  if (directory_name==NULL)
    directory_name = "SC_OUT_DEFAULT";
  ...
  snprintf(fn,sizeof(fn),"%d%s",sc_nb,filename);
