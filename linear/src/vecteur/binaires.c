 ...
 *
 * Vector v is updated to be equal to M v, where M is the identity
 * matrix except for its ith column that is replaced by s. In other
 * words, the resulting v is v + v_i s - v_i e_i = v + v_i (s - e_i),
 * where e_i is the ith base vector.
 *
 * Because the transformation is applied to a constraint system, it is
 * better to update s once, and not a this low-level. So this compute
 * v + v_i s.
 *
