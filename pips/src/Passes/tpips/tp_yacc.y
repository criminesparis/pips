/*
  ...
  $Id: tp_yacc.y 23412 2017-08-09 15:07:09Z irigoin $
...
%token TK_SHELL TK_ECHO TK_UNKNOWN TK_TIMEOUT
...
%type <status> i_timeout i_version
...
i_setenv: TK_SET_ENVIRONMENT TK_NAME TK_NAME TK_ENDOFLINE
	...
	| TK_SET_ENVIRONMENT TK_NAME TK_INT TK_ENDOFLINE
	{ char * v; asprintf(&v, "%d", $3); set_env_log_and_free($2, v);	}
	| TK_SET_ENVIRONMENT TK_NAME TK_EQUAL TK_INT TK_ENDOFLINE
	{ char * v; asprintf(&v, "%d", $4); set_env_log_and_free($2, v);	}
	;
...
i_create: TK_CREATE workspace_name /* workspace name */
    ...
    {
			if (workspace_exists_p($2))
				pips_user_error("Workspace %s already exists. Delete it!", $2);
			else if (db_get_current_workspace_name())
      {
          ...
          if (!ws_ok)
          {
            if (get_bool_property("CLOSE_WORKSPACE_AND_QUIT_ON_ERROR"))
            {
              ...
              pips_user_error("Could not create workspace \"%s\"", $2);
            }
            else
            {
              db_close_workspace(false);
...
i_source
  : TK_SOURCE filename_list TK_ENDOFLINE
  {
    process_file_list($2);
...
owner:	TK_OPENPAREN TK_OWNER_ALL TK_CLOSEPAREN
	...
	{
		pips_debug(7,"reduce rule owner (ALLFUNC)\n");
		if (tpips_execution_mode)
		{
			if (!db_get_current_workspace_name())
				pips_user_error("No current workspace! create or open one!\n");
        ...
        | TK_OPENPAREN TK_OWNER_ALLCU TK_CLOSEPAREN
	{
		pips_debug(7,"reduce rule owner (ALLCU)\n");
		if (tpips_execution_mode)
		{
			if (!db_get_current_workspace_name())
				pips_user_error("No current workspace! create or open one!\n");
	...
	{ $$ = $2; }
  | // No explicit argument, take current module if any
	{
