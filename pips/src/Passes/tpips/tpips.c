/*
  ...
  $Id: tpips.c 23628 2020-07-07 12:23:26Z coelho $
...
#define tpips_usage                                                     \
  ...
  "\t-x  xml-logfile: log to this file in XML.\n"                       \
...
static char *tp_help_topics[] =
{
  ...
  "checkpoint", "info", "show", "timeout", "checkactive", (char*) NULL
};
...
static char * tpips_read_a_line(char * main_prompt)
{
  ...
  // keep track of commands (there is also a log_file...)
  if (logfile && line)
    fprintf(logfile,"%s\n",line);
...
/* handle a user error
 * DO NOT TERMINATE here, see pips_log_end
 */
  ...
  va_list * args)
{
      ...
      get_bool_property("CLOSE_WORKSPACE_AND_QUIT_ON_ERROR"))
  {
    ...
    {
      pips_user_warning("Close workspace and quit on user error requested.");
...
/* In case of a tpips internal error, either core dump or try to save
 * the workspace.
*/
void tpips_internal_error(
  _UNUSED_ const char * calling_function_name,
  _UNUSED_ const char * a_message_format,
  ...)
{
  if (get_bool_property("CLOSE_WORKSPACE_AND_QUIT_ON_ERROR"))
  {
    // Try to close the workspace... hmmm, it may raise an internal error...
    db_close_workspace(true);
    // FI: it should be exit(1), but this is not compatible with the
    // current validation process. -- FC: why???
    exit(1);
  }
}
...
void tpips_help(string line)
{
  ...
  TP_HELP("timeout", "timeout <delay>\n",
          "\tset pips timeout in seconds\n");
...
void tpips_init(void)
{
  ...
  pips_internal_error_handler = tpips_internal_error;
...
static void tpips_exec(char * line)
{
  ...
  CATCH(any_exception_error)
  {
    ...
    {
      // hmmm... what should it do on timeout?
      // currently this is a clean "quit"... should it abort? other??
      tpips_close();
      pips_stop(user_error_log, TIMEOUT_CODE, "tpips stopped on timeout");
    }

    // on other errors, restart
...
int tpips_main(int argc, char * argv[])
{
  ...
  tpips_close();
  pips_stop(info_log, 0, "tpips end of main");
