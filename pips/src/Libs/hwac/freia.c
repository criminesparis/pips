/*
  ...
  $Id: freia.c 23495 2018-10-24 09:19:47Z coelho $
...
static void freia_cleanup_status(statement s, set helpers)
{
  ...
  // cleanup statements
  gen_context_recurse(s, &ctx, call_domain, fcs_call_flt, gen_null2);
...
static bool freia_image_shuffle(statement s, set shuffled)
{
  ...
  gen_context_recurse(s, &ctx, call_domain, fis_call_flt, gen_null2);
...
{
  int count = 0;
  gen_context_recurse(s, &count, call_domain, gen_true2, sctc_call_rwt);
...
static bool freia_alloc_call_p(const call c)
{
  ...
  const char *called = entity_user_name(f);
...
static bool freia_alloc_stat_p(const statement s)
{
  ...
  if (ENTITY_CONTINUE_P(f))
  {
    FOREACH(entity, var, statement_declarations(s))
    {
      ...
      {
        expression eval = value_expression(val);
        if (expression_call_p(eval) &&
            ! freia_alloc_call_p(expression_call(eval)))
          return false;
      }
...
{
  return i2a((int) statement_number((statement) s));
}
...
/**
 * @brief Transpose a structuring element expression
 */
...
/**
 * @brief Detect a call to a se transposition function and
 * transpose the arguments
 *
 * The expected function should have the following signature
 *   transpose_func(int kernelOut[9], const int kernelIn[9], ...)
 */
...
static bool fsi_seq_flt(sequence sq, freia_info * fsip)
{
  ...
  FOREACH(statement, s, lseq)
  {
    ...
    // the statement is kept if
    bool keep_stat =
      // it is an AIPO call
      freia_api ||
      // it follows kept calls and
      (ls && (
        // it is an (image) allocation in the middle of the code
        freia_alloc_stat_p(s) ||
        // or it has no image effects
        !freia_some_effects_on_images(s)));
    ...
    FOREACH(statement, s, rstats)
    {
      ...
      if (!is_freia_dealloc(s))
        gen_context_recurse(s, referenced,
                            ...
                            reference_domain, gen_true2, collect_images);
...
{
  set referenced = set_make(set_pointer);
