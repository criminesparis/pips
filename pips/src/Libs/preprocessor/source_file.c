/*
  ...
  $Id: source_file.c 23476 2018-07-13 08:53:55Z ancourt $
...
string preprocessor_current_split_file_name = string_undefined; // for warning messages
...
static bool pips_split_file(string name, string tempfile)
{
  ...
  preprocessor_current_split_file_name = strdup(name);
  ...
  free(dir);
  free(preprocessor_current_split_file_name);
...
static string process_thru_C_pp(string name) {
  ...
  if (eol_code>0)
    pips_user_error(
      "EOL encoding for file \"%s\" is \"%s\" and not supported\n",
...
/* Sed command to replace the second occurence of the formal parameter __x of the functions
     used twice on the same line by __y.
     Examples from math.h:
  ...
  */
    if (status==0) {
      status = safe_system_no_abort
	(concatenate("sed -i -e \"s/(long double __x) ;/(long double __y);/2g\" '",
		     new_name, "' 2> ", cpp_err, NULL));
...
static bool pips_check_syntax(string env, string prop)
{
  ...
  if (v && (*v=='n' || *v=='f'  || *v=='0' || *v=='N' || *v=='F' ))
    return false;

  // else look for the property
  ...
  string file_name, string compiler, string options, string language)
{
		   ...
		   " -c -o /dev/null", NULL)))
  {
    ...
    /*                   language, file_name); */
    /* Hopefully, this new warning is more informative and easier to
     * retrieve as last line in the Warnings file.
     */
    pips_user_warning("\n\%s syntax errors in source file \"%s\". "
		      "Check source file and/or compilation and preprocessing flags.\n",
                      language, file_name);
    syntax_ok_p = false;
  }
  free(pfile_name);
