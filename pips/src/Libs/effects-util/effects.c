/*
  ...
  $Id: effects.c 23412 2017-08-09 15:07:09Z irigoin $
...
/* Is it an anywhere effect (kind=0)? a typed anywhere effect (kind=1)
 * ? or any kind of anywhere effect (kind=2)?
 */
static bool generic_anywhere_effect_p(effect e, int kind)
{
  ...
  anywhere_p =  (entity_all_locations_p(v) && (kind==0||kind==2))
    || (entity_typed_anywhere_locations_p(v) && (kind==1||kind==2));
...
/* Is it a typed anywhere effect?  *ANYMMODULE*:*ANYWHERE*_b0, 1, 2 */
 ...
 *
 * This function works only for standard references, not for points-to
 * references, but is partially extended to cope with one anywhere...
 *
 * FI: I do not understand why pointers could be indexed in standard
 * references.
 */
