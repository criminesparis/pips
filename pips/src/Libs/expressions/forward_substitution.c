/*
  ...
  $Id: forward_substitution.c 23495 2018-10-24 09:19:47Z coelho $
...
{
  pips_assert("true", only_scalar==only_scalar);
            ...
            if(assignment_statement_p(first))
            {
                ...
                if (subs)
                {
                    ...
                    */
                    ...
                    {
                        param.stop=false;
                        gen_context_recurse(next, &param, statement_domain, do_substitute, gen_null2);
                        if(param.stop) break;
                        else once=true;
                    }
...
bool forward_substitute(const char* module_name)
{
    ...
    /* do the job here:
     */
    gen_context_recurse(get_current_module_statement(), dg,statement_domain, fs_filter, gen_null2);
