/*
  ...
  $Id: utils.c 23412 2017-08-09 15:07:09Z irigoin $
...
/**
 ...
 ALIAS_ACROSS_TYPES
 */
effect make_some_anywhere_effect(action act, entity anywhere_ent)
{
  // functions that can be pointed by reference_to_effect_func:
  // reference_to_simple_effect
  // reference_to_convex_region
  // reference_to_reference_effect
  effect anywhere_eff = (*reference_to_effect_func)(
...
effect make_anywhere_effect(action act)
{
  ...
  effect anywhere_eff = make_some_anywhere_effect(act, anywhere_ent);

  return anywhere_eff;
}

effect make_generic_anywhere_effect(action act, reference r)
{
  entity anywhere_ent = entity_undefined;
  if(get_bool_property("ALIASING_ACROSS_TYPES")) {
    anywhere_ent = entity_all_locations();
  }
  else {
    type t = points_to_reference_to_concrete_type(r);
    // FI: if t is not a pointer type, returns t...
    type pt = type_to_pointed_type(t);
    anywhere_ent = entity_typed_anywhere_locations(pt);
  }
  effect anywhere_eff = make_some_anywhere_effect(act, anywhere_ent);
...
/* Make a typed anywhere effect if possible according to ALIASING_ACROSS_TYPES */
effect make_typed_anywhere_effect(action act, type t)
{
  entity anywhere_ent = entity_undefined;
  if(get_bool_property("ALIASING_ACROSS_TYPES")) {
    anywhere_ent = entity_all_locations();
    ...
    anywhere_ent = entity_typed_anywhere_locations(t);
  }
  effect anywhere_eff = make_some_anywhere_effect(act, anywhere_ent);
...
*/
/* FI: this function is not used anymore, 13 July 2016 */
bool r_effect_pointer_type_p(effect eff, list l_ind, type ct)
{
	...
	case is_type_enum:
	  {
	    ...
	    pips_debug(7, "field case, with field expression : %s \n",
		       expression_to_string(field_exp));
...
/**
 ...
 FI: NOT FULLY IMPLEMENTED FOR ALL POSSIBLE ABSTRACT LOCATIONS
...
bool effect_pointer_type_p(effect eff)
{
  ...
  if (entity_abstract_location_p(ent)) {
    // FI: potentially, we have other abstract locations that are not typed
    p = true;
  }
  else {
    // p = r_effect_pointer_type_p(eff, l_ind, t);
    type et = points_to_reference_to_concrete_type(ref);
    p = pointer_type_p(et);
  }
 ...
 *
 * Filter out the memory/store write effects on pointers
 *
 * The list is/should be simplified when abstract locations are used.
 *
 * By definition, the untyped anywhere abstract location includes pointers.
 *
 * FI: this function is to be completed for abstract locations that
 * are not the all locations abstract location, and particularly for
...
list statement_modified_pointers_effects_list(statement s)
{
  ...
  FOREACH(EFFECT, eff, l_cumu_eff)
	...
	{
	  if (anywhere_effect_p(eff))
	    anywhere_p = true; // FI: why not break here?
	  else if (store_effect_p(eff) && effect_pointer_type_p(eff))
...
static bool effects_access_paths_comparable_p(effect eff1, effect eff2,
    int *result)
{
  bool comparable_p = true; /* assume they are comparable */
...
/* See if list el contain an abstract effect with action ea and type
 * et
 */
bool abstract_effect_in_effect_list_p(action ea, type et, list el)
{
  bool found_p = false;
  FOREACH(EFFECT, e, el) {
    if(action_equal_p(effect_action(e), ea)) {
      reference er = effect_any_reference(e);
      if(ENDP(reference_indices(er))) {
	entity ee = reference_variable(er);
	// FI: we are only interested in *ANY_MODULE*:*ANYWHERE*_bxx entities */
	if(entity_abstract_location_p(ee)) {
	  type eet = entity_type(ee); // FI: named type should not be used for abstract locations
	  if(type_equal_p(et, eet)) {
	    found_p = true;
	    break;
	  }
	}
    ...
    if (get_bool_property("EFFECTS_POINTER_MODIFICATION_CHECKING"))
    {
          ...
          ! (anywhere_w_p && anywhere_r_p))
      {
            ...
            !((eff_w_p && anywhere_w_p) || (!eff_w_p && anywhere_r_p)))
        {
	     ...
	     && effects_access_paths_comparable_p(eff, eff_p, &comp_res)
	     // FI: I want a write effect by eff_p on a prefix of eff
	     // && comp_res <=0 )
	     && comp_res <0 )
          {
	    if(!get_bool_property("EFFECTS_IGNORE_DEREFERENCING")) {
	      found = true;
	      if(get_bool_property("ALIASING_ACROSS_TYPES")) {
		...
		action ea = copy_action(effect_action(eff));
		reference er = effect_any_reference(eff);
		type et = points_to_reference_to_concrete_type(er);
		if(abstract_effect_in_effect_list_p(ea, et, l_res))
		  new_eff = effect_undefined;
		else
		  new_eff = make_generic_anywhere_effect(ea, er); 
	      }
	      if(!effect_undefined_p(new_eff))
...
list pointer_effects_to_constant_path_effects(list l_pointer_eff)
{
  ...
  FOREACH(EFFECT, eff, l_pointer_eff)
  {
    ...
    if(store_effect_p(eff))
    {
      ...
      // FI: should use package_effect_p() ?
      if (io_effect_p(eff)
          || malloc_effect_p(eff)
...
list effect_to_constant_path_effects_with_no_pointer_information(effect eff)
{
  ...
  if (effect_reference_dereferencing_p(ref, &exact_p)) {
    if(!get_bool_property("EFFECTS_IGNORE_DEREFERENCING")) {
	 ...
	 && std_file_entity_p(e)) {
	// Preserve the effect as such because it cannot create
	...
	effect neff =
	  make_generic_anywhere_effect(copy_action(effect_action(eff)), ref);
	le = CONS(EFFECT, neff, le);
      }
    }
  }
  else {
...
list effects_to_constant_path_effects_with_no_pointer_information(list el)
{
  list cel = NIL;
  FOREACH(EFFECT, ef, el) {
    cel = gen_nconc(cel, effect_to_constant_path_effects_with_no_pointer_information(ef));
  }
  return cel;
}

// moved here from accel-util
