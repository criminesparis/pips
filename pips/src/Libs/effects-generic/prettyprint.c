...
// for free_value_mappings()?
// ??? could/should be removed?
#include "transformer.h"
// for module_to_value_mappings()?
// ??? could/should be removed?
#include "semantics.h"
...
add_a_generic_prettyprint(
  ...
  generic_attachment_function ta)
{
  set_a_generic_prettyprint(resource_name, is_a_summary,
                            gen_chunk_undefined, tf, tp, ta);
}
    ...
    bool give_code_p, bool use_values)
{
	...
	set_proper_rw_effects((statement_effects)
				 db_get_memory_resource
				 (DBR_PROPER_EFFECTS, module_name, true));
...
{
  // remove consistency check which creates a generic -> convex dependency
  /*
