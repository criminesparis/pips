/*
  $Id: liveness_analysis_engine.c 23375 2017-05-11 08:58:06Z ancourt $
...
live_out_paths_from_test_to_branches(
  test t, _UNUSED_ live_paths_analysis_context *ctxt)
{
  statement
...
/*
static bool
live_paths_from_instruction_to_expression(
  instruction instr,
...
/**************************** GENERIC INTERPROCEDURAL LIVE_OUT REGIONS ANALYSIS */

/**
 * Create a new nsc system from the sc system of a region.
 * On this new nsc system only keep live variable.
 * /param sc        constraint system of the region
 * /param ref       reference variable that the region carry, have to keep on the new system of constraints
 * /param llivein   list of live in effect, variables to keep
 * /param lliveout  list of live out effect, variables to keep
 * /return          a new system of constraint for the region that only keep live variable
 */
