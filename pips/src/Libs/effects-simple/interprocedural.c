  ...
  for(c_eff = l_eff; !ENDP(c_eff); POP(c_eff)) {
      ...
      case is_storage_ram:
	{
	  ...
	  if (dynamic_area_p(ram_section(r))
	    ...
	    type ut = entity_basic_concrete_type(eff_ent);
	...
	break;
      case is_storage_rom:
	if(!entity_special_area_p(eff_ent)
	   && !anywhere_effect_p(eff)
	   // FI: this function does not exist, at least with this name
	   /* && !typed_anywhere_effect_p(eff)*/ )
	  ignore_this_effect = true;
	break;
          ...
          gen_length(real_ind) < type_depth(entity_type(real_ent)))
      {
        FOREACH(EFFECT, eff, l_sum_eff) {
          ...
          if (entity_all_locations_p(reference_variable(n_eff_ref)))
          {
            ...
            effect real_eff = (*reference_to_effect_func)(real_ref, copy_action(effect_action(eff)), true);
