/*
  ...
  $Id: translation.c 23412 2017-08-09 15:07:09Z irigoin $
...
/* FI: Let's deal with simple cases I think I know how to deal with.
 */
...
(reference input_ref, descriptor __attribute__ ((unused)) input_desc,
 ...
 bool *exact_p)
{
  list sl = reference_indices(address_of_ref);
  bool ok_p = nb_common_indices==0 || ENDP(sl);
  if(ok_p) {
	...
	*exact_p = add_points_to_subscript_lists(&osl, asl, CDR(isl));
	...
	if(type_equal_p(cit, cat)) {
	  ...
	  int nd = maximal_type_depth(rt);
	  // FI: may be "nd" when POINTS_TO_STRICT_POINTER_TYPES is set to true
	  bool strict_p = get_bool_property("POINTS_TO_STRICT_POINTER_TYPES");
	  if(strict_p? nis<=nd : nis<=nd+1) {
