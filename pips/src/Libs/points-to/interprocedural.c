...
// FI: just to access semantics_user_warning()
...

static bool new_recursive_filter_formal_context_according_to_actual_context
(cell fc, cell ac, approximation ap, set pt_in, set pt_binded, set binding, set filtered);

/* Handle constant cells such as NULL and UNDEFINED (nowhere) in "fcl"
 * and "acl"
 *
 * Update "fcl" is needed, as well as "pt_binded" and "filtered", a
 * subset of "pt_in".
 */
 ...
 *
 * For instance, if "ac" only points to the undefined cells, the call
 * site is not consistent because "fc" is assumed by the caller to
 * point to a defined cell. "ok_p" is set to false.
 *
 * if "fc" points to NULL in pt_in, but "ac" does not point to NULL,
 * the corresponding arc is not copied in "filtered".
 *
 * if "fc" points to "nfc" in "pt_in" and "ac" points to "nac" in
 * "pt_binded", then arc "fc->nfc" is copied in "pt_binded" and arc
 * "nfc->nac" is added in "binding".
 *
 * No optimization in number of scans for the different sets
*/
static bool new_recursive_filter_formal_context_according_to_actual_context_for_pointer_pair
(cell fc, cell ac, approximation ap, set pt_in, set pt_binded, set binding, set filtered)
{
  bool ok_p = true;
 ...
 * generic_points_to_cell_to_useful_pointer_cell() with no filtering
 * and a direct processing of the pairs of pointers obtained.
 *
...
/* List al and fpcl are assumed consistent, and consistent with the
