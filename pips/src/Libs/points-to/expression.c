/*
  ...
  $Id: expression.c 23415 2017-08-14 22:02:20Z irigoin $
...
#include "semantics.h" // for semantics_user_warning()
 ...
 *
 * If the source may point to null, the corresponding arc is
 * removed. If the source points only to null, the resulting graph is
 * bottom.
 *
 ...
 *
 * If set "pt" is defined, make sure that each cell in the returned
 * list, "pcl" appears in "pt".
 */
static list generic_points_to_cell_to_useful_pointer_cells(cell c, set pts)
{
  ...
  if(pointer_type_p(c_t) && cell_in_points_to_set_p(c, pts)) {
    cell nc = copy_cell(c);
	...
	// FI: possible memory leak for nc; 
	// If it is not a pointer, it should be freed
	// If it is a pointer, it might be useful or not
	list ppcl = generic_points_to_cell_to_useful_pointer_cells(nc, pts);
	pcl = gen_nconc(pcl, ppcl);
	// If nc is not in pcl, it should be free
      }
    }
  }
    ...
    pcl = generic_points_to_cell_to_useful_pointer_cells(nc, pts);
    // FI: should nc be freed or not ?
    // If nc is not in pcl, it should be free
  }
  return pcl;
}
