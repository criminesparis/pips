/*
  ...
  $Id: points_to_set.c 23412 2017-08-09 15:07:09Z irigoin $
...

/* Add a store independent points-to arc: source and destination
 * references include no dereferencing nor varying subscript such as
 * a[i].
 */
...
/* The source and destination references imply no dereferencing but
 * the subscripts may be any kind of expression, such as a[i].
 *
 * This is not possible in general, but may be useful at a specific
 * statement level. This used to handle precisely the backward
 * translation at call sites, especially for simple and convex effects
