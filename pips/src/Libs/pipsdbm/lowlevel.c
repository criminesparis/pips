/*
  ...
  $Id: lowlevel.c 23490 2018-10-22 09:49:27Z coelho $
...
/* rather approximated.
 * beware that this function is used to check whether a resource is a file,
 * and also to check its current time on pipsmake dependencies.
 */
bool displayable_file_p(const char* name)
{
  methods * m = get_methods(name);
  return
    // the resource itself is a string
    m->write_function==writeln_string &&
    // and the resource names ends with "_FILE"
    find_suffix((string) name, "_FILE");
}
