/*
  ...
  $Id: database.c 23412 2017-08-09 15:07:09Z irigoin $
...
// #include "ri-util.h" // compilation_unit_p, a string function, really
#include "naming.h"
...
static void dump_db_resource(
  const char* rname, const char* oname, db_resource r)
{
  ifdebug(1)
  {
...
int db_delete_obsolete_resources(bool (*keep_p)(const char*, const char*))
{
  ...
  pips_debug(4, "building list of resources to delete... (keep=%p)\n", keep_p);
  DB_RESOURCES_FOREACH(os, or, get_pips_database())
  {
    DB_OWNED_RESOURCES_FOREACH(rs, r, or)
    {
      string rn = db_symbol_name(rs), on = db_symbol_name(os);
  ...
  pips_debug(4, "deleting obsolete resources...\n");
  ...
  gen_free_list(lr);
  gen_free_list(lo);
  pips_debug(4, "deleting obsolete resources done.\n");
  debug_off();
  return ndeleted;
}
...
gen_array_t db_get_module_list_initial_order(void)
{
  ...
  db_resources dbr;
  gen_array_t a;
  // should this filtering out performed here? needed for UNSPLIT
  ...
  // first extract compilation units
  ...
  FOREACH(DB_SYMBOL, symbol, ls)
  {
    ...
    if (!string_undefined_p(name) &&
        ...
        bound_db_resources_p(dbr, symbol) &&
        !compilation_unit_p(name) &&
        (!ignore_rx || regexec(&ignore, name, 0, NULL, 0)))
      gen_array_dupappend(a, name);
  }
 ...
 * statements...
 *
 * @param module_p is used to select all modules, functions and
 * compilation units too. If true, compilation units are included as
 * well as functions.
 *
 * @param func_p is used to obtain either functions or compilation
 * units. It is only active if module_p is set to false.
 *
