/*
  ...
  $Id: for_to_do_or_while_loops.c 23470 2018-06-02 13:39:47Z coelho $
...
static bool guess_write_effect_on_entity_stmt_walker(statement st, guesser_param *p)
{
    FOREACH(ENTITY,e,statement_declarations(st))
        if( !value_undefined_p(entity_initial(e) ) ) {
            gen_context_recurse(entity_initial(e),p,call_domain, guess_write_effect_on_entity_walker,gen_null2);
        }
    return true;
}
 ...
 *
 * Assuming loop_index is a potential do loop index, and init a comma
 * expression, do we have subexpressions after the loop_index
 * initialization that may use this index? If yes, the loop cannot be
 * transformed into a DO loop, because this subexpression will be
 * moved before the loop index initialization. Or the loop index
 * initialization should be replicated before the loop. 
 *
 * Example (SPEC2006, milc):
 *
 *   for (i = parity==0x01?even_sites_on_node:0, s = &lattice[i];
 *        i<(parity==0x02?even_sites_on_node:sites_on_node);
 *        i++, s++) {
 *
 ...
 *
 * - the loop is converted into a while loop with the initialization
 *   expression moved into a statement (easy, alreayd implemented,
 *   safe)
 *
 * - the conversion into a DO loop is performed, but *all*
 *   initializations are moved into a statement and the new
 *   initialization expression is "loop_init = loop_init" to satisfy the
 *   do loop semantics (a new implementation is required).
 */
bool guess_late_read_effect_on_entity(expression init, entity loop_index)
{
  ...
  if(expression_call_p(init)) {
    ...
    if(ENTITY_COMMA_P(f)) {
      bool write_found_p = false;
      list el = call_arguments(c);
      FOREACH(EXPRESSION, e, el) {
	if(!write_found_p && guess_write_effect_on_entity(e, loop_index))
	  write_found_p = true;
	else if(write_found_p) {
	  list rl = expression_to_reference_list(e, NIL);
	  FOREACH(REFERENCE, r, rl) {
	    guess_p = reference_variable(r)==loop_index;
	    if(guess_p)
	      break;
	  }
	  if(guess_p) {
	    gen_free_list(rl); // Just the spine...
	    break;
	  }
	}
      }
...
sequence for_to_do_loop_conversion(forloop theloop, statement parent)
{
    ...
    SET_FOREACH(entity,loop_index,cond_inter_incr_entities)
    {
      ...
      if(scalar_integer_type_p(lit) /* && type_depth(lit)==1*/ ) {
	   ...
	   &&!guess_late_read_effect_on_entity(init, loop_index))
        {
