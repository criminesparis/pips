/*
  ...
  $Id: loop_normalize.c 23495 2018-10-24 09:19:47Z coelho $
...
#include "ri.h"
#include "ri-util.h"
#include "workspace-util.h" // for make_max_exp

#include "control.h" // module_reorder
#include "transformations.h" // find_loop_from_label in "index_set_splitting"
...
/** Apply the loop normalization upon a module
    ...
    @return true
*/
bool loop_normalize(const string mod_name) {
  /* prelude */
...
bool linearize_loop_range(const char *module_name) {
    ...
    if (!empty_string_p(loop_label)) {
      ...
      gen_context_recurse(get_current_module_statement(), &did_something,
          statement_domain, gen_true2, do_linearize_loop_range);
    }
    if(did_something) {
        module_reorder(get_current_module_statement()); ///< we may have had statements
