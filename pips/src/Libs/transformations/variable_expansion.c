/*
  ...
  $Id: variable_expansion.c 23495 2018-10-24 09:19:47Z coelho $
...
#include "phases.h"
#if defined(BUILDER_VARIABLE_EXPANSION) ||      \
  defined(BUILDER_REDUCTION_VARIABLE_EXPANSION)
...
#include "effects.h"
#include "reductions.h" // used
#include "control.h" // module_reorder

#include "transformations.h" // make_ref_expr, find_loop_from_label
...
bool prepare_expansion(loop l, scalar_expansion_context* ctxt)
{
    ...
    dimension d = make_dimension(int_to_expression(0), make_op_exp("-",range_to_expression(r,range_to_nbiter),int_to_expression(1)), NIL);
...
void do_expand_reference_in_declarations(statement s, entity oni[])
{
    FOREACH(ENTITY,e,statement_declarations(s))
    {
        ...
        if( !value_undefined_p(v) && value_expression_p( v ) )
            gen_context_recurse(v, oni, reference_domain, gen_true2, do_expand_reference);
    }
}
    ...
    if(entity_undefined_p(elabel)) {
        ...
        if(!ENDP(reductions))
        {
	      ...
	      make_dimension(int_to_expression(0),
			     make_op_exp(MINUS_OPERATOR_NAME,loop_nbiters,int_to_expression(1)),
			     NIL);
...
#endif // BUILDER_ * _VARIABLE_EXPANSION
