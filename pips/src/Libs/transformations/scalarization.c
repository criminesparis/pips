/*
  ...
  $Id: scalarization.c 23495 2018-10-24 09:19:47Z coelho $
...
#include "phases.h"
#if defined(BUILDER_SCALARIZATION) ||               \
  defined(BUILDER_CONSTANT_ARRAY_SCALARIZATION) ||  \
  defined(BUILDER_QUICK_SCALARIZATION)
...
{
  const char * test_s =
      get_string_property("SCALARIZATION_ACROSS_CONTROL_TEST");
 ...
 *
 * MA: It probably shouldn't be located here (seems very "linear" oriented)
 */
static Psysteme sc_add_offset_variables(Psysteme g, Pbase b, Pbase db) {
  ...
  if (base_included_p(b,gb)
    ...
    for(c = eqs ; !CONTRAINTE_UNDEFINED_P(c) ; c = contrainte_succ(c)) {
      ...
      for (cb=b,cdb=db ;
           !BASE_NULLE_P(cb) ;
           cb = vecteur_succ(cb), cdb = vecteur_succ(cdb)) {
    ...
    */
    if (SC_EMPTY_P(g1) || sc_empty_p(g1)) {
      ...
      for ( cr = dr ; !BASE_UNDEFINED_P(cr) ; cr = vecteur_succ(cr) ) {
        ...
        if (!(feasible_p
              && value_eq(VALUE_ZERO, pmin)
              && value_eq(VALUE_ZERO, pmax))) {
          functional_p = false;
          break;
        }
      }
  ...
  if (declaration_statement_p(st)) {
    FOREACH(ENTITY, decl, statement_declarations(st)) {
      ...
      if (! value_undefined_p(init_val)) {
        gen_context_recurse(init_val, ctx,
          reference_domain, reference_substitute, gen_null2);
      }
    }
  }
  return true;
...
static void statement_substitute_scalarized_array_references(statement st,
                                                             reference pvr,
                                                             entity s) {
...
static effect  effect_write_or_read_on_variable(list el,
                                                entity v,
                                                bool write_p) {
  ...
  if(v) {
    FOREACH(EFFECT, e, el) {
      if (store_effect_p(e)) {
        ...
        if(same_entity_p(ev, v)
            && (write_p ? action_write_p(a) : action_read_p(a))) {
          return(e);
        }
      }
    }
...
static void scalarize_variable_in_statement(entity pv,
  ...
  const char* epref = strlen(dpref)==0?
                      ...
                      : dpref;
  entity sv = make_new_scalar_variable_with_prefix(epref,
  ...
  if (get_bool_property("SCALARIZATION_FORCE_OUT")
    ...
    statement co_s = make_assign_statement(reference_to_expression(pvr),
...
struct scalarization_ctx {
  ...
  // Do not scalarize "&vars..."
  bool skip_address_of_variables;
};
...
static bool loop_scalarization(loop l, struct scalarization_ctx *ctx)
{
  ...
  bool memory_in_out_regions_only_p =
      get_bool_property("MEMORY_IN_OUT_EFFECTS_ONLY");
  ...
  FOREACH (EFFECT, pr, crwl) {
    ...
    entity iv = (entity) gen_find(pv,
                                  irl,
                                  (bool(*)()) gen_eq,
                                  car_effect_to_variable);
    // Does the current variable appear in the out effect?
    entity ov = (entity) gen_find(pv,
                                  orl,
                                  (bool(*)()) gen_eq,
                                  car_effect_to_variable);
...
static bool reference_constant_wrt_ctxt_p(reference ref,
  ...
  if (reference_variable(ref) == ctxt->e) {
    FOREACH(EXPRESSION, exp, reference_indices(ref)) {
      ...
      FOREACH(EFFECT, eff_exp, l_eff_exp) {
        ...
        /* if the indice contains a reference to an array element, we assume it
         * is not constant
         * we could use the cumulated effects of the current statement for more
         * precision but as we could not scalarize the array because the region
         * would not contain a single element, it's of no use here.
         */
...
static bool declarations_reference_constant_wrt_ctxt_p(statement st,
                                                       reference_testing_ctxt *p_ctxt) {
  if (declaration_statement_p(st)) {
    FOREACH(ENTITY, decl, statement_declarations(st)) {
      value init_val = entity_initial(decl);
      if (! value_undefined_p(init_val)) {
        gen_context_recurse(init_val, p_ctxt,
                            reference_domain, reference_constant_wrt_ctxt_p, gen_null2);
      }
    }
  }
  return true;
...
static bool statement_entity_references_constant_in_context_p(statement s,
                                                              entity e,
  ...
  if (declaration_statement_p(s))
      ...
      FOREACH(ENTITY, e, l_decls)
	{
	  gen_context_recurse(entity_initial(e), &ctxt,
				    call_domain, entity_accessed_in_call_in, gen_null2);
	}
    }
  return (!ctxt->used_through_external_calls);
}
      ...
      if (descriptor_convex_p(d)) {
        ...
        if (!effect_undefined_p(pru)) { // FI: this should be an assert?
          ...
          if (nvo != neo) {
            ...
            if(threshold<2) {
              pips_user_error("The scalarization threshold should be at "
...
static void aov_call(call c, aov_ctx * ctx)
{
  entity called = call_function(c);
  if (ENTITY_ADDRESS_OF_P(called))
  {
    list args = call_arguments(c);
    pips_assert("1 arg to &", gen_length(args) == 1);
    if (set_belong_p(ctx->seen, EXPRESSION(CAR(args))))
    {
      ctx->found = true;
...
static void aov_cast(cast c, aov_ctx * ctx)
{
  expression e = cast_expression(c);
  if (set_belong_p(ctx->seen, e))
  {
    expression enclosing =
      (expression) gen_get_current_ancestor(expression_domain);
    if (enclosing)
      set_add_element(ctx->seen, ctx->seen, enclosing);
  }
}

static void aov_ref(reference r, aov_ctx * ctx)
{
...
// return whether "&v" exists in statement s
// this is not really efficient, maybe some data structure could be used
// to keep track of that... but we want in statement s *only*...
static bool address_of_variable_in_statement(statement s, entity v)
{
  aov_ctx ctx;
  ctx.var = v;
  ctx.found = false;
  ctx.seen = set_make(set_pointer);

  gen_context_multi_recurse(s, &ctx,
                            call_domain, gen_true, aov_call,
                            cast_domain, gen_true, aov_cast,
                            reference_domain, gen_true, aov_ref,
                            NULL);
...
static void scalarization_statement_out(statement s,
  ...
  if(!declaration_statement_p(s) /*&& !statement_call_p(s)*/
    ...
    if(!ENDP(local_scalarized_variables)) {
      ...
      {
        if (ctx->skip_address_of_variables &&
            address_of_variable_in_statement(s, pv))
        {
          pips_debug(4, "skipping variable %s\n", entity_name(pv));
          continue;
        }
...
bool scalarization(const string module_name)
{
  entity module;
  ...
  set_cumulated_rw_effects((statement_effects) db_get_memory_resource(DBR_REGIONS,
                                                                      module_name,
                                                                      true));
  set_proper_rw_effects((statement_effects) db_get_memory_resource(DBR_REGIONS,
                                                                      module_name,
                                                                      true));
  ...
  set_transformer_map((statement_mapping) db_get_memory_resource(DBR_TRANSFORMERS,
                                                                 module_name,
                                                                 true));
  ...
  struct scalarization_ctx ctx;
  ctx.keep_perfect_parallel =
    get_bool_property("SCALARIZATION_KEEP_PERFECT_PARALLEL_LOOP_NESTS");
  ctx.blacklisted_loops = set_make(set_pointer);
  ctx.skip_address_of_variables =
    get_bool_property("SCALARIZATION_SKIP_ADDRESS_OF_VARIABLES");
  ...
  if (false) {
    ...
    ifdebug(1) {
      pips_assert("scalarized_variables is empty",
                  ENDP(ctx.scalarized_variables));
...
static void all_array_references_constant_walker(reference ref,
                                                 references_constant_param* p) {
  if(same_entity_p(p->array,reference_variable(ref))) {
    expression offset = reference_offset(ref);
    p->constant_p &= extended_expression_constant_p(offset)
...
static bool all_array_references_constant_p(statement in, entity array) {
  ...
  gen_context_recurse(in, &p,
                      reference_domain,
                      gen_true2,
                      all_array_references_constant_walker);
  return p.constant_p;
}
...
static void replace_constant_array_references_walker(reference ref,
                                                     replace_references_constant_param *p) {
  if(same_entity_p(p->array,reference_variable(ref))) {
...
static void replace_constant_array_references(statement in, entity array) {
  replace_references_constant_param p = { array,
      hash_table_make(hash_int, HASH_DEFAULT_SIZE) };
  gen_context_recurse(in, &p,
                      reference_domain,
                      gen_true2,
...
bool constant_array_scalarization(const string module_name)
{
  set_current_module_entity(module_name_to_entity(module_name));
  set_current_module_statement((statement) db_get_memory_resource(DBR_CODE,
                                                                  module_name,
...
static bool scalarizable_entity_p(entity e) {
  ...
  pips_debug(3,
             "checking entity %s, with storage %s \n",
 ...
 */
static bool statement_compute_scalarizable_candidates(statement st,
  ...
  if (statement_loop_p(st)) {
    ...
    /* then scan the loop_body declarations because they do not belong to
     * cumulated effects */
  ...
  if(ENDP(prefix)) {
    if(!ENDP(ls)) {
      ...
      FOREACH(STATEMENT, st, ls) {
        ...
        set scalarizable_candidates =
            (set) hash_get(loops_scalarizable_candidates,
                           (char *) statement_loop(st));
        set_del_element(scalarizable_candidates, scalarizable_candidates, e);
        pips_debug(1, "Variable %s is removed from scalarizable candidates of "
            "statement %td\n",
...
static void update_scalarizable_candidates(vertex v, statement st,
  ...
  ifdebug(1) {
    if(statement_loop_p(st)) {
      pips_debug(1, "Trying to scalarize %s in loop statement %td "
          "(ordering %03zd)",
  ...
  FOREACH(SUCCESSOR, succ, vertex_successors(v)) {
    ...
    dg_vertex_label succ_l = (dg_vertex_label)vertex_vertex_label( succ_v ) ;
    dg_arc_label arc_l = (dg_arc_label)successor_arc_label( succ ) ;
    statement succ_st = ordering_to_statement(dg_vertex_label_statement(succ_l));
    instruction succ_i = statement_instruction( succ_st ) ;
    list succ_ls = load_statement_enclosing_loops( succ_st ) ;
    list prefix = gen_common_prefix( ls, succ_ls ) ;
        ...
        */
        if(action_write_p(effect_action(sk_eff))) {
          ...
          FOREACH(statement, s, prefix) {
            loop l = statement_loop(s);
            ifdebug(3) {
              fprintf(stderr, "%s ", entity_name(loop_index(l)));
            }
          ...
          for(;
              keep && !ENDP(sc_inds) && !ENDP(sk_inds);
              POP(sc_inds), POP(sk_inds)) {
            expression sc_exp = EXPRESSION(CAR(sc_inds));
            expression sk_exp = EXPRESSION(CAR(sk_inds));
            if (unbounded_expression_p(sc_exp) ||
                unbounded_expression_p(sk_exp)) {
              keep = false;
            } else {
        ...
        if (!keep) {
          ...
          /* e cannot be a local variable at a lower level than
             ...
             arc. */
          remove_scalarizable_candidate_from_loops(prefix,
                                                   ls,
                                                   e,
                                                   loops_scalarizable_candidates);
          remove_scalarizable_candidate_from_loops(prefix,
 ...
 */
static bool entity_can_be_scalarized_in_statement_in(statement s,
    ...
    FOREACH(EFFECT, eff, l_eff) {
      ...
      if (store_effect_p(eff) && eff_e == ctxt->e) {
        if (reference_undefined_p(ctxt->first_ref)) {
          ...
          pips_assert("all scalarizable references have the same number "
              "of indices",
                ...
                list l_modified_variables)
{
  ...
  */
  ...
  if (nvo != neo) {
    pips_debug(2,
               "First legality criterion not met: %d!=%d (nvo!=neo)\n",
               nvo, neo);
    result = false;
  } else {
    scalarizability_test_ctxt ctxt;
    ...
    gen_context_recurse(s, &ctxt,
       statement_domain, entity_can_be_scalarized_in_statement_in, gen_null2);
    result = ctxt.result;
    if (!result) {
      ...
      // check that the reference indices do not depend on variables
      // modified in the statement
      ifdebug(3) {
...
*/
static void check_loop_scalarizable_candidates(loop l,
                                               hash_table loops_scalarizable_candidates) {
  set loop_scalarizable_candidates = hash_get(loops_scalarizable_candidates, l);
  statement body = loop_body(l);
  list l_loop =
      CONS(STATEMENT, (statement) gen_get_ancestor(statement_domain, l),NIL);
  list l_modified_variables =
      effects_to_written_scalar_entities(load_cumulated_rw_effects_list(body));
  ...
  SET_FOREACH(entity, e, loop_scalarizable_candidates) {
    if (!entity_can_be_scalarized_in_statement_p(e,
                                                 body,
                                                 l_modified_variables)) {
...
*/
static void check_scalarizable_candidates(statement s,
                                          hash_table loops_scalarizable_candidates) {
  gen_context_recurse(s, loops_scalarizable_candidates,
                      loop_domain, gen_true2, check_loop_scalarizable_candidates);
}
 ...
 */
static void loop_scalarize_candidates(loop l,
                                      hash_table loops_scalarizable_candidates) {
  set loop_scalarizable_candidates = hash_get(loops_scalarizable_candidates, l);

  /* be deterministic! */
  list l_loop_scalarizable_candidates =
      set_to_sorted_list(loop_scalarizable_candidates,
 ...
 */
static void scalarize_candidates(statement s,
                                 hash_table loops_scalarizable_candidates) {
  gen_context_recurse(s, loops_scalarizable_candidates,
                      loop_domain, gen_true2, loop_scalarize_candidates);
}

bool quick_scalarization(const string module_name)
{
  entity module;
  statement module_stat;
  ...
  gen_context_recurse(module_stat, loops_scalarizable_candidates,
          statement_domain, statement_compute_scalarizable_candidates, gen_null2);
  ...
  FOREACH(VERTEX, v, graph_vertices( dependence_graph )) {
    ...
    statement st = ordering_to_statement(dg_vertex_label_statement(vl));
    ...
    FOREACH(EFFECT, eff, load_proper_rw_effects_list( st )) {
      ...
      if( action_write_p(effect_action(eff)) && array_effect_p(eff)) {
        update_scalarizable_candidates( v, st, eff,
                                        loops_scalarizable_candidates) ;
      }
    }
  }
   ...
   */
  /* first check if candidates are really scalarizable
   * with scalarize_variable_in_statement*/
...
#endif // BUILDER_ * _ SCALARIZATION
