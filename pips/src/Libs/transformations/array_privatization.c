/*
  ...
  $Id: array_privatization.c 23412 2017-08-09 15:07:09Z irigoin $
...
#include "phases.h"
#if defined(BUILDER_ARRAY_PRIVATIZER) ||            \
  defined(BUILDER_ARRAY_SECTION_PRIVATIZER) ||      \
  defined(BUILDER_PRINT_CODE_PRIVATIZED_REGIONS) || \
  defined(BUILDER_DECLARATIONS_PRIVATIZER)
...
#include "pipsmake.h" // ??? find_rule_by_resource hack
...
static void array_privatization_error_handler(void)
{
...
static bool privatizer(const string module_name);
 ...
 */
bool array_privatizer(const string module_name)
{
    store_as_regions = false;
    store_as_loop_locals = true;
    privatize_sections = false;
 ...
 * comment  : local regions can contain local variables.
 */
bool array_section_privatizer(const string module_name)
{
...
static bool privatizer(const string module_name)
{
    ...
    reset_hooks_register(array_privatization_error_handler);
    ...
    if (store_as_regions)
    {
      DB_PUT_MEMORY_RESOURCE(DBR_PRIVATIZED_REGIONS,
                             module_name, (char*) get_private_effects());
      DB_PUT_MEMORY_RESOURCE(DBR_COPY_OUT_REGIONS,
                             module_name, (char*) get_copy_out_effects());
    }
    ...
    reset_hooks_unregister(array_privatization_error_handler);
...
bool print_code_privatized_regions(const string module_name)
{
...
privatize_entity(entity ent)
{
	...
	new_ent_storage = make_storage
	    (is_storage_ram,
	     make_ram(module,
		      dynamic_area,
		      current_offset_of_area(dynamic_area,
					     new_ent),
		      NIL));
...
bool declarations_privatizer(const string mod_name)
{
