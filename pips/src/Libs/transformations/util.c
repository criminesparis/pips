...
statement find_loop_from_label(statement s, entity label)
{
  ...
  gen_context_recurse(s, &ctx,
                      loop_domain, find_loop_from_label_walker, gen_null2);
  return ctx.found;
}

static bool clean_statement_label(statement st)
{
  entity e = statement_label(st);
  const char* label_pattern = get_string_property("LABEL_EXCEPTION");
  bool exception = (*label_pattern!=0) && (strstr(entity_user_name(e),label_pattern)!=NULL);
  if(!entity_empty_label_p(e) &&  !exception) {
...
bool clean_labels(const string mod_name)
{
  ...
  gen_recurse (mod_stmt, statement_domain, clean_statement_label, gen_null);
