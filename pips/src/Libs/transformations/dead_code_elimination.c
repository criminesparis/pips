/*
  ...
  $Id: dead_code_elimination.c 23495 2018-10-24 09:19:47Z coelho $
...
static bool statement_call_a_keep_function_p( statement s ) {
  ...
  if(statement_call_p(s) || statement_expression_p(s)) {
    gen_context_recurse(s,&user_function_found, call_domain, match_call_to_user_function, gen_null2);
  }
...
remove_all_the_non_marked_statements(statement s)
{
  ...
  gen_context_recurse(s, entities_to_remove,
      /* Since statements can be nested, only remove in a
                  bottom-up way: */
      statement_domain, gen_true2, remove_this_statement_if_useless);
