/*
  $Id: simplify_control.c 23495 2018-10-24 09:19:47Z coelho $
...
static bool discard_statement_and_save_label_and_comment(statement s)
{
  if (statement_loop_p(s)) {
    /* NN -> Bug found : if we have two loops with the same label such
       as :
...
static bool dead_statement_filter(statement s)
{
  ...
  for(;;) {
     ...
     * of code.
     *
     * But STOP, exit(), abort() statements, guarded or not by tests,
     * hidden or not by function calls, can also introduce
     * discontinuities...  Well, to please Ronan let's put a fast
     * accurate enough test for that last case.
     *
...
static bool remove_useless_label_internal(statement s)
{
  ...
  gen_context_recurse(s, &changed,
      statement_domain, gen_true2, statement_remove_useless_label);
