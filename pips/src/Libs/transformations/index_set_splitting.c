/*
  ...
  $Id: index_set_splitting.c 23495 2018-10-24 09:19:47Z coelho $
...
#include "phases.h"
#if defined(BUILDER_INDEX_SET_SPLITTING) ||     \
  defined(BUILDER_FORCE_LOOP_FUSION)
...
#include "transformations.h" // find_loop_from_label
...
bool index_set_splitting(const string module_name)
{
    /* prelude */
...
static bool try_loop_fusion(statement s0, statement s1) {
    ...
    if(range_equal_p(loop_range(l0),loop_range(l1))) {
        hash_table renaming = hash_table_make(hash_pointer,HASH_DEFAULT_SIZE);
        do {
...
static void do_loop_fusion(entity lbl) {
    gen_context_recurse(get_current_module_statement(), lbl,
	    sequence_domain, gen_true2, do_loop_fusion_walker);
}

/* loop_fusion */
bool force_loop_fusion(const string module_name)
{
    /* prelude */
    set_current_module_entity(module_name_to_entity( module_name ));
    set_current_module_statement((statement) db_get_memory_resource(DBR_CODE, module_name, true) );
