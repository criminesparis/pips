/*
  ...
  $Id: freeze_variables.c 23495 2018-10-24 09:19:47Z coelho $
...
#include "phases.h"
#ifdef BUILDER_FREEZE_VARIABLES
...
bool freeze_variables(char *mod_name) 
{
  ...
  if (!statement_undefined_p(mod_stmt)) {
    ...
    if (!ENDP(new_le.old))
      gen_context_recurse(mod_stmt, &new_le, 
	  statement_domain, gen_true2, freeze_variables_in_statement);
...
#endif // BUILDER_FREEZE_VARIABLES
