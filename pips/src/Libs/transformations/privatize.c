/*
  $Id: privatize.c 23495 2018-10-24 09:19:47Z coelho $
...
#include "phases.h"
#if defined(BUILDER_PRIVATIZE_MODULE) ||            \
  defined(BUILDER_PRIVATIZE_MODULE_EVEN_GLOBALS) || \
  defined(BUILDER_LOCALIZE_DECLARATIONS)
...
#include "transformations.h" // module_clean_declarations in "declarations.c"
...
static bool entity_privatizable_in_loop_statement_p(
  entity e, statement stmt, bool even_globals)
{
  ...
  */
           ...
           && top_level_entity_p( e) /* this test may be removed since we check that e is used as a loop index */)
  {
    ...
    gen_context_recurse(loop_body(l), &ctxt, loop_domain, loop_in, gen_null2);
...
static void try_privatize(vertex v, statement st, effect f, entity e)
{
  ...
  ifdebug(1) {
    if(statement_loop_p(st)) {
      ...
      print_entities(loop_locals(statement_loop(st)));
    }
    else {
      ...
      if(store_effect_p(sc)) {
        ...
        if (v==succ_v) {
          ...
          pips_debug(5,"remove %s from locals in non common enclosing loops\n",
                     entity_local_name(e));
          prefix = gen_common_prefix( ls, succ_ls ) ;
...
static bool generic_privatize_module(string mod_name, bool even_globals)
{
  entity module;
  statement mod_stat;
  graph mod_graph;
...
bool privatize_module(const string mod_name)
{
  return generic_privatize_module(mod_name, false);
}

bool privatize_module_even_globals(const string mod_name)
{
...
static set gather_loop_indices(void *v) {
  ...
  gen_context_recurse(v, s, loop_domain, gen_true2, do_gather_loop_indices);
  return s;
}
 ...
 * @return
 */
bool localize_declaration(const string mod_name)
{
  /* prelude */
  ...
  ifdebug(1) {
    pips_debug(1,"The statement before we convert loop_locals "
