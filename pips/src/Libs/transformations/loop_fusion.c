/*
 ...
 $Id: loop_fusion.c 23495 2018-10-24 09:19:47Z coelho $
...
static bool fine_fusion_loops(statement sloop1,
  ...
  if(success && get_bool_property("LOOP_FUSION_KEEP_PERFECT_PARALLEL_LOOP_NESTS")
    ...
    if(!statement_undefined_p(inner_loop1) && !statement_undefined_p(inner_loop2)) {
      ...
      if(loop_parallel_p(statement_loop(inner_loop1)) &&
        ...
        gen_context_recurse(inner_loop1,stmts1,statement_domain,record_statements,gen_null2);

        set stmts2 = set_make(set_pointer);
        gen_context_recurse(inner_loop2,stmts2,statement_domain,record_statements,gen_null2);
  ...
  if(success) {
    ...
    if(index1!=index2) {
      ...
      gen_context_recurse(body_loop2, &thecouple,
              statement_domain, gen_true2, replace_entity_effects_walker);
    }
    // Cleaning FIXME
    loop_body(loop1) = body_loop1;
    gen_free_list(fused);
...
static fusion_block make_block_from_statement(statement s, int num) {
  ...
  gen_context_recurse(s, b->statements, statement_domain, record_statements, gen_null2);
...
static void compute_fusion_on_statement(statement s, bool coarse) {
  ...
  gen_context_recurse( s, &params, sequence_domain, fusion_in_sequence, gen_null2);
}
