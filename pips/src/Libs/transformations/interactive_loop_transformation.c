/*
  ...
  $Id: interactive_loop_transformation.c 23495 2018-10-24 09:19:47Z coelho $
...
#include "conversion.h" // look_for_nested_loop_statements
#include "pipsmake.h" // safe_make hack?

#include "prettyprint.h" // extension_to_string

#include "transformations.h" // find_loop_from_label in "util.c"

// forward declaration
/*
static char* loop_pragma(const char*, const char*);
static char* module_loops(const char*, const char*);
*/
...
bool selected_loop_p(statement s)
{
  ...
  return statement_loop_p(s) &&
          (statement_label(s) == selected_label ||
           loop_label(statement_loop(s)) == selected_label);
}
...
bool flag_loops(const string module_name)
{
  ...
  /* run loop labeler */
  gen_context_recurse(get_current_module_statement(), &flp,
		      statement_domain, gen_true2, flag_loop);
...
char* module_loops(const char* module_name, const char* parent_loop)
{
  ...
  // run loop gatherer
  gen_context_recurse(seed, &loops,
		      statement_domain, module_loops_walker, gen_null2);
