 ...
 *
 * Note: the subscripts of the region reference are not updated
 *
 * See also region_value_substitute(), all_regions_variable_rename()
 */
...
/**
 ...
 @param tac is an action tag (read or write)
             tac don't have to be use by another effects (make a copy)
...
effect make_reference_region(reference ref, action tac)
{
  ...
  else
  {
    ...
    if (d>0 || pointer_p)
    {
      ...
      pips_debug(8, "pointer or array or struct case \n");
	...
	if(!ENDP(sl)) {
	  ...
	  POP(sl);
	  if(field_expression_p(s))
	    ns = copy_expression(s);
	  else
	    ns = make_phi_expression(dim);
	}
	else {
	  // As it may occur in Fortran IO for instance
	  ns = make_phi_expression(dim);
	}
      ...
      for (idim = 1; ind != NIL; idim++, ind = CDR(ind))
      {
        ...
        bool dim_linear_p = false;
        bool unbounded_p =  unbounded_expression_p(exp_ind);
	bool field_p = field_expression_p(exp_ind);
        ...
        ifdebug(3)
        {
          if (unbounded_p)
            ...
            pips_debug(3, "addition of equality :\nPHI%d - %s = 0\n",
		       idim, expression_to_string(exp_ind));
        }
...
/* Returns the number of a phi entity, i.e. phi =
 * make_phi_entity(phi_entity_rank(phi))
 *
 * rank in [1.. NB_MAX_ARRAY_DIM], which may not be enough for
 * points-to references...
 *
 * In case of failure, return 0.
 */
int phi_entity_rank(entity phi)
{

  int rank = 0;
  // We could also check the package name
  string ln = (string) entity_local_name(phi);
  if(strncmp(ln, PHI_PREFIX, 3)==0) {
    string rn = ln+3;
    int n = sscanf(rn, "%d", &rank);
    if(n!=1)
      rank = 0;
  }
  return rank;
...
/* Returns the list of phi variables used as subscript in effect eff */
list convex_effect_to_phi_entity_list(effect eff)
{
  list phi_l = NIL;
  reference r = effect_any_reference(eff);
  list sl = reference_indices(r);
  FOREACH(EXPRESSION, se, sl) {
    syntax ss = expression_syntax(se); //subscript syntax
    if(syntax_reference_p(ss)) {
      reference sr = syntax_reference(ss);
      entity phi = reference_variable(sr);
      int r = phi_entity_rank(phi);
      if(r>0)
	phi_l = CONS(ENTITY, phi, phi_l);
    }
  }
  phi_l = gen_nreverse(phi_l);
  return phi_l;
}
			 ...
			 bool is_phi_first)
{
  if(expression_range_p(expr)) {
	...
	if (!effects_private_current_context_stack_initialized_p()
	  ...
	  // FI: this should be checked for memory leaks
	  prec = transformer_identity();
	else
	  {
	    transformer p = effects_private_current_context_head();
	    // No need for information about previous variable updates
	    prec = transformer_range(p);
	  }
	...
	// FI: prec should be freed here I believe

	must_p = false;

	/* Careful: side-effects are lost */
	if (!transformer_undefined_p(trans)) {
	  if(!ENDP(transformer_arguments(trans))) {
	    string e = expression_to_string(expr);
	    semantics_user_warning(
        "Side-effects in subscript expression \"%s\" should be avoided:\n", e);
	    free(e);
	  }
	  transformer new_trans = transformer_temporary_value_projection(trans);

	  /* Is the phi variable constrained by an equation? an
	   * inequality? nothing?
	   */
	  if(transformer_equations_constrain_variable_p(new_trans, phi))
	    must_p = true;
	  else if(transformer_inequalities_constrain_variable_p(new_trans, phi)) {
	    string e = expression_to_string(expr);
	    semantics_user_warning(
        "Imprecise subscript expression \"%s\" should be avoided:\n", e);
	    free(e);
	  }
	  else {
	    string e = expression_to_string(expr);
	    semantics_user_warning(
        "No information derived from subscript expression \"%s\":\n", e);
	    free(e);
	  }


	  /* Add the new constraint to the convex array region being built. */
