/*
  ...
  $Id: interface.c 23372 2017-05-05 15:35:30Z lossing $
...
/******************************************* INTERFACES FOR LIVENESS ANALYSIS/FILTER */

bool live_out_regions(const char* module_name)
{
  bool ok;
//  if (! c_module_p(module_name_to_entity(module_name))
//      || !get_bool_property("CONSTANT_PATH_EFFECTS"))
...
//  set_methods_for_live_paths(module_name);
  // ambiguous if we have to use simple or convex for the second arg
  // anyway, it's not use.
  ok = live_out_region_engine(module_name, simple);
  return ok;
}
...
bool
print_code_live_out_regions(const char* module_name)
{
  return print_code_any_regions(module_name, is_inout, false, false,
      DBR_LIVE_OUT_REGIONS, string_undefined, ".loutreg");
}

//bool
//print_source_live_out_regions(const char* module_name)
//{
//  return print_code_any_regions(module_name, is_inout, true, false,
//      DBR_LIVE_OUT_REGIONS, string_undefined, ".uloutreg");
//}
