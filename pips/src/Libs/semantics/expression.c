/*
  ...
  $Id: expression.c 23625 2020-07-07 06:46:31Z coelho $
...
#include "points-to.h" // 2 functions
#else // no HAVE_PIPS_points_to_LIBRARY
// typedef set points_to_graph
static points_to_graph get_points_to_graph_from_statement(
...
static list expression_to_points_to_sinks(
...
static list expression_to_points_to_sources(
  _UNUSED_ expression e,
  _UNUSED_ points_to_graph in)
{
  pips_internal_error("points-to library not available");
...
// static: no longer because used in ri_to_transformers.c because
    ...
    bool is_internal)
{
  ...
  if(ENTITY_UNARY_MINUS_P(op)) {
    ...
    list ll = semantics_expression_to_points_to_sinks(e1);
     ...
     * and in struct_refrence_assignment_to_transformer()...
     */
    transformer ntf = transformer_undefined;
    FOREACH(CELL, cp, ll) {
      reference r = cell_any_reference(cp); // rhs location
      ...
      if(semantics_usable_points_to_reference_p(r, e1, n)) {
	entity l = constant_memory_access_path_to_location_entity(r);
	if(!entity_undefined_p(l)) {
	  rt = transformer_add_equality(rt, e, l);
	}
      }
      else {
	free_transformer(rt);
	rt = transformer_undefined;
      }
						     ...
						     bool is_internal)
{
  ...
  // FI: the types of subexpressions e1 and e2 should be checked
  // because they might be pointers or a combination of pointer and
  // integer. Equations must be adapted by multiplying entities with
  // sizeof() constants. Not clear how to do this with symbolic
  // sizeofs used by Nelson to improve portability.
  ...
  case is_syntax_cast:
    {
      ...
      cast cce = syntax_cast(s);
      expression ce = cast_expression(cce);
      newpre =
	transformer_add_condition_information_updown(pre, ce, context,
						     veracity, upwards);
        break;
    }
  default:
...
static transformer modulo_of_a_constant_to_transformer(entity e, int lb1, int lb2, int ub2)
{
  ...
  transformer tf3 = transformer_undefined;
	...
	if(lb2==0) {
	  ...
	  semantics_user_warning("Zero divide encountered\n");
 ...
 *
 * See if exist a set v_i such that v2 = sum_i a_i v_i + c. Then v2 can
 * be rewritten as v2 = gcd_i(a_i) lambda + c.
 *
 * if k divides gcd(a_i) then v2>0 ? v1 = v2 % k = c % k : v1 = v2 % k = c%k-k
 *
 * If the sign of v2 is unknown, c%k-k<=v1<=c%k
 *
 */
    ...
    bool is_internal)
{
  ...
  if(transformer_empty_p(t1) || transformer_empty_p(t2)) {
    // Some exception occurs during the evaluation of e1 or e2
    // For instance, a zero divide
    tf = t1;
    t1 = transformer_undefined;
  }
  else if(!transformer_undefined_p(t1) && !transformer_undefined_p(t2)) {
    int lb1 = 0;
    int ub1 = 0;
    int lb2 = 0;
    int ub2 = 0;
    ...
    transformer p1 = transformer_range(transformer_apply(pre, t1));
    //t2 = transformer_range(transformer_apply(pre, t2));
    transformer p2 = transformer_range(transformer_combine(copy_transformer(t1), t2));
    ...
    // FI-> Nelson: there may be a problem, but your are reanalyzing
    //e1 and e2, which has just been performed above; and you fail when side
    // effects occur (multiply07.c)
    //integer_expression_and_precondition_to_integer_interval(e1, p1, &lb1, &ub1);
    //integer_expression_and_precondition_to_integer_interval(e2, p2, &lb2, &ub2);
    integer_value_and_precondition_to_integer_interval(v1, p1, &lb1, &ub1);
    integer_value_and_precondition_to_integer_interval(v2, p2, &lb2, &ub2);
    //free_expression(ev1);
    //free_expression(ev2);
    ...
    if(lb1==ub1) {
      ...
      tf = transformer_combine(t1, t2);
      transformer_equality_add(tf, veq);
      t1 = transformer_undefined;
      //free_transformer(t1);
    }
    else if(lb2==ub2) {
      ...
      tf = transformer_combine(t1, t2);
      tf = transformer_equality_add(tf, veq);
      t1 = transformer_undefined;
      //free_transformer(t2);
    }
...

/* This function is redundant with generic_unary_operation_to_transformer()
 * except for its use of parameter is_pointer
 */
...
struct interval_product {int lb1; int ub1; int lb2; int ub2; bool positive_p;};

/* Auxiliary function for non-affine operators such as divide,
 * multiply, modulo...
 *
 * Returns an equivalent set of interval products whose bounds are
 * always positive plus some information about the global sign for the
 * result. For instance, [-3,4] x [-5,6] is broken down into four products:
 *  - [1,3] x [1,5] positive
 *  - [1,3] x [0,6] negative
 *  - [0,4] x [1,5] negative
 *  - [0,4] x [0,6] positive
 *
 * For division, 0 is excluded and the positive intervals start at 1
 * instead of 0.
 *
 * Intervals may be empty: [1,0].
 */

/* DEAD CODE
static void break_intervals_into_positive_intervals(int lb1, int ub1, int lb2, int ub2, struct interval_product cases[4], int * pncase, bool exclude_zero_p)
{
  // We need a pair of macros to process -MAX_INT and -MIN_INT,
  // MINUS_UPPER_BOUND and MINUS_LOWER_BOUND, because of the disymetry
  // in the encoding of integers
  //

  int nlb = exclude_zero_p? 1 : 0;
  if(false) {
  *pncase = 0;
  if(lb1>=0) {
    // The first interval is positive
    if(lb2>=0) {
      *pncase = 1;
      cases[0].lb1 = lb1, cases[0].ub1 = ub1, cases[0].lb2 = lb2, cases[0].ub2 = ub2, cases[0].positive_p = true; 
    }
    else if(ub2<=0) {
      *pncase = 1;
      cases[0].lb1 = lb1, cases[0].ub1 = ub1, cases[0].lb2 = -ub2, cases[0].ub2 = -lb2, cases[0].positive_p = false; 
    }
    else {
    // The second interval must be broken into two sub-intervals
      *pncase = 2;
      cases[0].lb1 = lb1, cases[0].ub1 = ub1, cases[0].lb2 = 1, cases[0].ub2 = -lb2, cases[0].positive_p = false; 
      cases[1].lb1 = lb1, cases[1].ub1 = ub1, cases[1].lb2 = nlb, cases[1].ub2 = ub2, cases[1].positive_p = true; 
    }
  }
  else if(ub1<=0) {
    ...
    if(lb2>=0) {
      *pncase = 1;
      cases[0].lb1 = -ub1, cases[0].ub1 = -lb1, cases[0].lb2 = lb2, cases[0].ub2 = ub2, cases[0].positive_p = false; 
    }
    else if(ub2<=0) {
      *pncase = 1;
      cases[0].lb1 = -ub1, cases[0].ub1 = -lb1, cases[0].lb2 = -ub2, cases[0].ub2 = -lb2, cases[0].positive_p = true; 
    }
    else {
    // The second interval must be broken into two sub-intervals
      *pncase = 2;
      cases[0].lb1 = -ub1, cases[0].ub1 = -lb1, cases[0].lb2 = 1, cases[0].ub2 = -lb2, cases[0].positive_p = true; 
      cases[1].lb1 = -ub1, cases[1].ub1 = -lb1, cases[1].lb2 = nlb, cases[0].ub2 = ub2, cases[1].positive_p = false; 
    }
  }
  else {
    // The first interval must be broken into two sub-intervals
    if(lb2>=0) {
      *pncase = 2;
      cases[0].lb1 = 1, cases[0].ub1 = -lb1, cases[0].lb2 = lb2, cases[0].ub2 = ub2, cases[0].positive_p = false; 
      cases[1].lb1 = nlb, cases[1].ub1 = ub1, cases[1].lb2 = lb2, cases[1].ub2 = ub2, cases[1].positive_p = true; 
    }
    else if(ub2<=0) {
      *pncase = 2;
      cases[0].lb1 = 1, cases[0].ub1 = -lb1, cases[0].lb2 = -ub2, cases[0].ub2 = -lb2, cases[0].positive_p = true; 
      cases[1].lb1 = 1, cases[1].ub1 = ub1, cases[1].lb2 = -ub2, cases[1].ub2 = -lb2, cases[1].positive_p = false; 
    }
    else {
    // The second interval must be broken into two sub-intervals
      *pncase = 4;
      cases[0].lb1 = 1, cases[0].ub1 = -lb1, cases[0].lb2 = 1, cases[0].ub2 = -lb2, cases[0].positive_p = true; 
      cases[1].lb1 = nlb, cases[1].ub1 = ub1, cases[1].lb2 = 1, cases[1].ub2 = -lb2, cases[1].positive_p = false; 
      cases[2].lb1 = 1, cases[2].ub1 = -lb1, cases[2].lb2 = nlb, cases[2].ub2 = ub2, cases[2].positive_p = false; 
      cases[3].lb1 = nlb, cases[3].ub1 = ub1, cases[3].lb2 = nlb, cases[3].ub2 = ub2, cases[3].positive_p = true; 
    }
  }
  }
  else {
    if(lb2>=0) {
      *pncase = 1;
      cases[0].lb1 = 1, cases[0].ub1 = 1, cases[0].lb2 = lb2, cases[0].ub2 = ub2, cases[0].positive_p = true; 
    }
    else if(ub2<=0) {
      *pncase = 1;
      cases[0].lb1 = 1, cases[0].ub1 = 1, cases[0].lb2 = -ub2, cases[0].ub2 = -lb2, cases[0].positive_p = false; 
    }
    else {
    // The v2 interval must be broken into two sub-intervals
      *pncase = 2;
      cases[0].lb1 = 1, cases[0].ub1 = 1, cases[0].lb2 = 1, cases[0].ub2 = -lb2, cases[0].positive_p = false; 
      cases[1].lb1 = 1, cases[1].ub1 = 1, cases[1].lb2 = nlb, cases[1].ub2 = ub2, cases[1].positive_p = true; 
    }
  }
}
*/

/* Equation v = v1/v2 for v1>=0 and v2>=0: v1-v2+1 <= v2 v <= v1
 *
 * We know positive bounds [lb1,ub1] for v1 and [lb2,ub2] for v2.
 ...
 *
 * v1-v2+1 <= ub2 v       => -ub2 v + v1 - v2 + 1 <= 0
 *            lb2 v <= v1 => lb2 v -v1 <= 0
 *
 * If -v must be used instead of v:
 *
 * v1-v2+1 <= -ub2 v       => ub2 v <= v2-v1-1 => ub2 v + v1 - v2 + 1 <= 0
 *            -lb2 v <= v1 => lb2 v >= v1      => -lb2 v -v1 <= 0
 *
 * Else, we can also eliminate v1 and v2 everywhere.
 * Substitute v1 thanks to lb1 <= v1 <= ub1:
 * lb1-v2+1 <= v2 v <= ub1
 * Substitue the left v2 in the same way:
 * lb1-ub2+1 <= v2 v <= ub1
 * If lb2==ub2, v2 v is an affine term.
 * Else substitue the central v2 for each inequality:
 *             lb2 v <= ub1 => v<= ub1/lb2
 * lb1-ub2+1 <=ub2 v        => v>= lb1/ub2
 *
 * if the positive flag is not set, v must be replaced by -v:
 *            -ub2 v <= ub1 => v>=-ub1/lb2
 * lb1-ub2+1<=-lb2 v        => v<=-lb1/ub2
 */
/* DEAD CODE
static transformer process_interval_for_divide(
    entity v, entity v1, entity v2, struct interval_product one_case)
{
  long long int lb1 = one_case.lb1;
  long long int ub1 = one_case.ub1;
  long long int lb2 = one_case.lb2;
  long long int ub2 = one_case.ub2;
  bool positive_p = one_case.positive_p;
  // Two possible options: true and false
  bool full_elimination = false;
  transformer itf = transformer_identity(); // no information at first

  if(full_elimination) {
    if(ub1 != INT_MAX && lb2 != INT_MAX) {
      if(positive_p)
	itf = transformer_add_inequality_with_integer_constraint(itf, v, ub1/lb2, true);
      else
	itf = transformer_add_inequality_with_integer_constraint(itf, v, -(ub1/lb2), false);
    }

    if(lb1 != INT_MAX && ub2 != INT_MAX && lb2 != INT_MAX) {
      if(positive_p)
	itf = transformer_add_inequality_with_integer_constraint(itf, v, lb1/ub2, false);
      else
	itf = transformer_add_inequality_with_integer_constraint(itf, v, -(lb1/ub2), true);
    }
  }
  else {
    // Minimize the number of substitutions:
    //
    // If v is positive because v1>=0 and v2 >=0
    //
    // v1 = v2 v + x ^ 0<=x<=v2-1  if v1 >= 0 && v2 >= 0
    // v1 <= v2 v + v2 -1 ^ v1>= v2 v ^ lb2 <= v2 <= ub2
    // v1-v2+1 <= ub2 v       => -ub2 v + v1 - v2 + 1 <= 0
    //            lb2 v <= v1 => lb2 v -v1 <= 0
    //
    // If v is negative because v1 <=0 and v2 >=0:
    //
    // v1 = v2 v + x ^ -v2+1<=x<=0  if v1 <= 0 && v2 >= 0
    // v1 <= v2 v ^ v1>= v2 v - v2 + 1
    // v1 <= lb2 v        => -lb2 v + v1 <= 0
    //            ub2 v -v2 +1 <= v1 => ub2 v -v1 -v2 + 1 <= 0
    //
    // If v2 <= 0, use -v2 and -v1?
    if(positive_p) {
      itf = transformer_add_3d_affine_constraint(itf, -ub2, v, 1, v1, -1, v2, 1, false);
      itf = transformer_add_3d_affine_constraint(itf, lb2, v, -1, v1, 0, v2, 0, false);
    }
    else {
      itf = transformer_add_3d_affine_constraint(itf, ub2, v, 1, v1, -1, v2, 1, false);
      itf = transformer_add_3d_affine_constraint(itf, -lb2, v, -1, v1, 0, v2, 0, false);
...
static transformer process_intervals_for_divide(
  transformer tf, entity v, entity v1, entity v2,
  struct interval_product cases[4], int ncase)
{
  int i;
  transformer dtf = transformer_empty();

  if(false) {
    // Perform the convex hull before the intersection is faster but
    // less accurate in general, maybe not when v1 and v2 have been fully
    // eliminated.
    for(i=0; i<ncase; i++) {
      transformer itf = process_interval_for_divide(v, v1, v2, cases[i]);
      dtf = transformer_convex_hull(dtf, itf);
    }
    // To be checked for functionality and memory leaks
    tf = transformer_image_intersection(tf, dtf);
  }
  else {
    for(i=0; i<ncase; i++) {
      transformer itf = process_interval_for_divide(v, v1, v2, cases[i]);
      transformer ctf = transformer_combine(copy_transformer(tf), itf);
      free_transformer(itf);
      // memory leak
      transformer ndtf = transformer_convex_hull(dtf, ctf);
      free_transformer(dtf);
...
/* The equation v1 = v2 v + r is always true. But we need to eliminate
 * the remainder r which is implicit and to linearize the product v2 v
 * to obtain affine constraints of v, v1, v2, using constant lb1, ub1,
 * lb2 and ub2, with lb1 <= v1 <= ub1 and lb2 <= v2 <=ub2.
 */
static transformer process_bounds_for_divide(transformer tf, entity v, entity v1, int lb1, int ub1, entity v2, int lb2, int ub2)
{
  if(lb2==0&&ub2==0) { // zero divide
    semantics_user_warning("Zero divide:\n");
    free_transformer(tf);
    tf = transformer_empty();
  }
  else if(lb2>=0) { // division by a strictly positive number
    if(lb2==0)
      lb2 = 1;
    if(lb1>=0) { // of a positive number: divide01
      /* v2 is positive and r is positive: 0 <= r <= v2 - 1
       * v1 = v2 v + r
       * v1 <= v2 v + v2 - 1
       * v1 >= v2 v
       * lb2 <= v2 <= ub2 => lb2 v <= v2 v <= ub2 v
       * => v1 <= ub2 v + v2 -1 => -ub2 v + v1 - v2 + 1 <= 0
       * => v1 >= lb2 v         =>  lb2 v - v1 <=0
       */
      if(ub2!=INT_MAX)
	tf = transformer_add_3d_affine_constraint(tf, -ub2, v, 1, v1, -1, v2, 1, false);
      else
	tf = transformer_add_3d_affine_constraint(tf, -1, v, 0, v1, 0, v2, 0, false);
      tf = transformer_add_3d_affine_constraint(tf, lb2, v, -1, v1, 0, v2, 0, false);
    }
    else if(ub1<=0) { // of a negative number: divide02
      /* v2 is positive and r is negative: -v2 + 1 <= r <= 0
       * v1 = v2 v + r
       * v1 <= v2 v         
       * v1 >= v2 v - v2 + 1
       * lb2 <= v2 <= ub2 => ub2 v <= v2 v <= lb2 v
       * => v1 <= lb2 v         => -lb2 v + v1 <= 0
       * => v1 >= ub2 v -v2 + 1 =>  ub2 v - v1 -v2 + 1 <=0
       */
      tf = transformer_add_3d_affine_constraint(tf, -lb2, v, 1, v1, 0, v2, 0, false);
    ...
    else { // of a number of unknown sign
      /* v2 is positive and r is bounded: -v2 + 1 <= r <= v2 - 1
       * v1 = v2 v + r
       * v1 <= v2 v + v2 - 1
       * v1 >= v2 v - v2 + 1
       *  I do not know how to linearize v2 v, unless v2 is a constant,
       * but v1/v2 can be bounded:
       * lb1 <= v1 <= ub1 => lb1/v2 <= v1/v2 <= ub1/v2 => lb1/lb2 <= v <= ub1/lb2
       *
       * Also, when v2 is a numerical constant, if exists x s.t. v1 ==
       * v2 x, then v == x. This is not exploited here.
       */
      if(lb2==ub2) { // v2 v is a linear term, Semantics-New/S124, divide07
	// - v2 v + v1 - v2 + 1 <=0
	tf = transformer_add_3d_affine_constraint(tf, -lb2, v, 1, v1, -1, v2, 1, false);
	// v2 v - v1 - v2 +1 <=0
	tf = transformer_add_3d_affine_constraint(tf, lb2, v, -1, v1, -1, v2, 1, false);
      }
      if(lb1!=INT_MIN)
	tf = transformer_add_3d_affine_constraint(tf, -1, v, 0, v1, 0, v2, lb1/lb2, false);
      if(ub1!=INT_MAX)
	tf = transformer_add_3d_affine_constraint(tf, 1, v, 0, v1, 0, v2, ub1/lb2, false);
    }
  }
  else if(ub2<=0) { // division by a strictly negative number
    if(ub2==0)
      ub2 = -1;
    if(lb1>=0) { // of a positive number, e.g. 100/-3 = -33 (r=1), divide03
      /* v2 is negative and r is positive: 0 <= r <= -v2 - 1
       * v1 = v2 v + r
       * v1 <= v2 v - v2 - 1
       * v1 >= v2 v
       * lb2 <= v2 <= ub2 => ub2 v <= v2 v <= lb2 v (since v is negative)
       * => v1 <= lb2 v - v2 -1 => -lb2 v + v1 + v2 + 1 <= 0
       * => v1 >= ub2 v         =>  ub2 v - v1 <=0
       */
      if(lb2!=INT_MAX)
	tf = transformer_add_3d_affine_constraint(tf, -lb2, v, 1, v1, 1, v2, 1, false);
	...
	tf = transformer_add_3d_affine_constraint(tf, 1, v, 0, v1, 0, v2, 0, false);
      tf = transformer_add_3d_affine_constraint(tf, ub2, v, -1, v1, 0, v2, 0, false);
    }
    else if(ub1<=0) {// of a negative number, e.g. -100/-3 = 33 (r=-1), divide04
      /* v2 is negative and r is negative: v2 + 1 <= r <= 0
       * v1 = v2 v + r
       * v1 <= v2 v         
       * v1 >= v2 v + v2 + 1
       * lb2 <= v2 <= ub2 => lb2 v <= v2 v <= ub2 v (since v is positive)
       * => v1 <= ub2 v          => -ub2 v + v1 <= 0
       * => v1 >= lb2 v + v2 + 1 =>  lb2 v - v1 + v2 + 1 <=0
       */
      tf = transformer_add_3d_affine_constraint(tf, -ub2, v, 1, v1, 0, v2, 0, false);
	...
	tf = transformer_add_3d_affine_constraint(tf, -1, v, 0, v1, 0, v2, 0, false);
      ...
      /* v2 is negative and r is bounded: v2 + 1 <= r <= -v2 - 1
       * v1 = v2 v + r
       * v1 <= v2 v - v2 - 1 => -v2 v + v1 + v2 + 1 <= 0
       * v1 >= v2 v + v2 + 1 => v2 v - v1 + v2 + 1 <=0
       * I do not know how to linearize v2 v, but v1/v2 can be bounded:
       * lb1 <= v1 <= ub1 => lb1/v2 >= v1/v2 >= ub1/v2 => ub1/ub2 <= v <= lb1/ub2
       */
      if(lb2==ub2) {
	tf = transformer_add_3d_affine_constraint(tf, -lb2, v, 1, v1, 1, v2, 1, false);
	tf = transformer_add_3d_affine_constraint(tf, lb2, v, -1, v1, 1, v2, 1, false);
      ...
      if(ub1!=INT_MAX)
	tf = transformer_add_3d_affine_constraint(tf, -1, v, 0, v1, 0, v2, ub1/ub2, false);
      if(lb1!=INT_MIN)
	tf = transformer_add_3d_affine_constraint(tf, 1, v, 0, v1, 0, v2, -lb1/ub2, false);
    }
  }
  else { // No sign information is available for v1 or v2
    /* We are left with dividing interval [lb1, ub1] by interval [lb2,
     * ub2] knowing that lb1 and lb2 are negative and ub1 and ub2
     * positive. Since 1 and -1 belongs to [lb2, ub2], the upper bound
     * is max(ub1, -lb1) and the lower bound is min(-ub1, lb1).
     */
    if(lb1!=INT_MIN && ub1!=INT_MAX) { // divide05, divide06
      int u = ub1>=-lb1? ub1: -lb1;
      int l = -ub1<=lb1? -ub1: lb1;
      // l <= v <= u
      tf = transformer_add_3d_affine_constraint(tf, 1, v, 0, v1, 0, v2, -u, false);
      tf = transformer_add_3d_affine_constraint(tf, -1, v, 0, v1, 0, v2, l, false);
    }
  }
  return tf;
}
							   ...
							   bool is_internal)
{
  ...
  if(ENTITY_PLUS_UPDATE_P(op) || ENTITY_MINUS_UPDATE_P(op)) {
    ...
    if(!get_bool_property("SEMANTICS_ASSUME_POSITIVE_REMAINDERS")) {
      /* Prologue more or less necessary for all non affine operators */
      entity v = e; // Formal parameter name should be fixed
      entity v1 = make_local_temporary_value_entity(entity_type(v));
      transformer pre1 = transformer_undefined_p(pre)? transformer_identity() : transformer_range(pre);
      transformer t1 = safe_integer_expression_to_transformer(v1, e1, pre1, is_internal);
      entity v2 = make_local_temporary_value_entity(entity_type(v));
      transformer pre2 = transformer_range(transformer_safe_apply(t1, pre1));
      transformer t2 = safe_integer_expression_to_transformer(v2, e2, pre2, is_internal);

      if(transformer_empty_p(t1) || transformer_empty_p(t2)) {
	// Some exception occurs during the evaluation of e1 or e2
	// For instance, a zero divide or an exit in a called function
	tf = transformer_empty();
      }
      else if(!transformer_undefined_p(t1) && !transformer_undefined_p(t2)) {
	int lb1 = 0, ub1 = 0, lb2 = 0, ub2 = 0;
	tf = transformer_combine(copy_transformer(t1), t2);
	/* post1 == pre2... */
	transformer post1 = transformer_range(transformer_apply(pre1, t1));
	transformer post2 = transformer_range(transformer_apply(post1, t2));
	// transformer post2 = transformer_range(tf);
	integer_value_and_precondition_to_integer_interval(v1, post1, &lb1, &ub1);
	integer_value_and_precondition_to_integer_interval(v2, post2, &lb2, &ub2);
      
	/* Case disjunction to use only positive intervals + sign information */
	//struct interval_product cases[4];
	//int ncase = 0;
	//break_intervals_into_positive_intervals(lb1, ub1, lb2, ub2, cases, &ncase, true);
	//tf = process_intervals_for_divide(tf, v, v1, v2, cases, ncase);
	tf = process_bounds_for_divide(tf, v, v1, lb1, ub1, v2, lb2, ub2);
      }
      else {
    ...
    bool is_internal)
{
  ...
  // arity unknown, but greater than 2?
  ...
  // arity = 2
    ...
    if((ENTITY_POINT_TO_P(f) || ENTITY_FIELD_P(f))
       && constant_path_analyzed_p()) {
      tf = lhs_expression_to_transformer(e, expr, pre);
    }
    else {
      expression e1 = EXPRESSION(CAR(args));
...
//	       entity_name(op));
  ...
  if(transformer_undefined_p(tf)) {
    semantics_user_warning("Not tested for logical.\n");
    //tf = points_to_unary_operation_to_transformer(v, op, arg, pre, is_internal, false);
    tf = generic_unary_operation_to_transformer(v, op, arg, pre, is_internal);
  }
    ...
    bool is_internal)
{
  ...
  if(ENTITY_C_MIN_P(op)) {
    ...
    default:
      /* min, max,... */
      semantics_user_warning("Operator %s not analyzed\n", entity_name(op));
    }
  }
...
static bool have_null_value_in_pointer_expression_p(expression expr) {
  ...
  switch (syntax_tag(sexpr)) {
    case is_syntax_reference:
    {
      ...
      if (basic_pointer_p(b)) {
        semantics_user_warning("TODO : Need to check if %s is indirectly NULL\n", expression_to_string(expr));
    ...
    bool is_internal)
{
  ...
  if(ENTITY_DEREFERENCING_P(op)) {
    // tf = generic_unary_operation_to_transformer(e, op, e1, pre, is_internal);
    ; // Peformed at the end of the function
  }
  else if(ENTITY_UNARY_MINUS_P(op)) {
    tf = unary_minus_operation_to_transformer(e, e1, pre, is_internal);
  }
  else if(ENTITY_POST_INCREMENT_P(op) || ENTITY_POST_DECREMENT_P(op)
            || ENTITY_PRE_INCREMENT_P(op) || ENTITY_PRE_DECREMENT_P(op)) {
    ...
    if (syntax_reference_p(s1)) {
      ...
      entity a1 = constant_memory_access_path_to_location_entity(r1);
      if(entity_undefined_p(a1)) {
	tf = transformer_identity(); // transformer_undefined might work too
      }
      else {
	// FI: is a1 usable ? 
	if(entity_has_values_p(a1)) {
    ...
    bool is_internal)
{
  ...
  if(ENTITY_MINUS_C_P(op)) {
    ...
    semantics_user_warning("ASSIGN for rhs pointer not tested : %s\n", expression_to_string(expr));
...

/* The value of the expression is irrelevant, but its sub-expressions
 * may generate a transformer.
 *
 * FI: use Newgen to reimplement this using abstract transformers to
 * simplify the recursion ?
 */
transformer any_expression_side_effects_to_transformer(
			      expression e,
			      transformer p,
			      bool is_internal __attribute__ ((unused)))
{
  ...
  case is_syntax_reference: {
      ...
      list el = reference_indices(r);
      //tf = any_expressions_side_effects_to_transformer(el, p, is_internal);
      tf = expressions_to_transformer(el, p);
    };
    break;
  case is_syntax_range: {
  ...
  case is_syntax_cast: {
      ...
      tf = any_expression_side_effects_to_transformer(ce, p, is_internal);
      break;
    };
  case is_syntax_sizeofexpression: {
      sizeofexpression soe = syntax_sizeofexpression(s);
      if(sizeofexpression_expression_p(soe)) {
	expression soee = sizeofexpression_expression(soe);
	tf = any_expression_side_effects_to_transformer(soee, p, is_internal);
      }
      break;
    };
  case is_syntax_subscript: {
      subscript ss = syntax_subscript(s);
      expression ae = subscript_array(ss);
      list sel = subscript_indices(ss);
      tf =
	any_expression_side_effects_to_transformer(ae, p, is_internal);
      transformer etf =
	any_expressions_side_effects_to_transformer(sel, p, is_internal);
  ...
  case is_syntax_application: {
      ...
      tf = any_expression_side_effects_to_transformer(fe, p, is_internal);
      transformer etf =
	any_expressions_side_effects_to_transformer(al, p, is_internal);
      tf = transformer_combine(tf, etf);
      free_transformer(etf);
      break;
    };
  case is_syntax_va_arg: {
      // FI: what could be useful ? Explore the sizeofexpressions ?
      break;
    };
  default:
    pips_internal_error("Unexpected or unimplemented tad=%d", syntax_tag(s));
    break;
  }
  return tf;
}

/* same as any_expression_side_effects_to_transformer() but for a list
 * of expressions
 */
...
/* Same as any_expression_side_effects_to_transformer() but effects
 * are used to always generate a transformer.
 *
 * FI: I do not know how effects should be used. It might be useful to
 * allow special transformers using location abstraction to simplify
 * the recurrence. Currently, effects are used or not. It might be
 * necessary to use only some of them at a given time in the
 * evaluation process as variables can be assigned almost anywhere in
 * C.
 */
transformer safe_any_expression_side_effects_to_transformer(
			      expression e,
			      transformer p,
			      bool is_internal)
{
 ...
 *
 * Or to capture the return of a struct, which cannot be expressed
 * with an atomic return value.
 */
...
/* See if references rlhs is usable and process null, undefined and
 * anywhere locations defined by rlhs.
 *
 * Expression lhs is passed to clarify the error and warning messages.
 *
 * n is the number of alternative references. It is used to check the
 * severity of unusable references. If n=1, a NULL rlhs implies a
 * bug. If n>1, the points-to analysis could not determine precisely
 * the target location.
 *
 * This function is designed to check constant references in a loop
 * over a list of references returned by the points-to analysis.
*/
...
/* This function deals with e1.e2 and e1->e2, not with simple
 * references such as x or a[i].
 *
 * This piece of code has mainly been retrieved from
 * any_assign_to_transformer().
 *
 * FI: I am not sure how to factorize it out because entity e
 * corresponds to some memory location and is modified in an
 * assignement environment. Here, e is likely to be a temporary
 * value. It is not updated. It should not appear among the
