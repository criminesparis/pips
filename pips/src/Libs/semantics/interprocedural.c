...
{
  bool go_down = false;
  // FI: necessary for points-to, (potentially?) useful for warnings and errors
...
#define PROCESS_CALL_DEBUG_LEVEL 5
  ...
  if(constant_path_analyzed_p()) {
    caller_prec = substitute_stubs_in_transformer(caller_prec, c, current_statement, false);
    caller_prec = substitute_formal_array_elements_in_precondition(caller_prec, current_callee, call_arguments(c), caller_prec, NIL);
  }
 ...
 * interprocedurally unless an array is passed as parameter.
 *
