/*
  ...
  $Id: ri_to_transformers.c 23650 2021-03-10 10:04:56Z coelho $
 ...
 * \param e         write effect (assumed), on a concrete location (assumed)
 * \param rhs       some expression. more or less linked to the effect,
 *                  useful if error r warning messages are emitted
...
/* Effect e is assumed to be a memory write effect on an array element
 * location. This must be checked by the caller.
 *
 ...
 *                  Only the write effects are taken into account.
 *                  A normalized empty transformer is not modified. 
 * \param tf        transformer (modified by side effect)
 ...
 *
 * Note that information about variables declared within the block is
 * not projected here, but in statement_to_transformer().
 *
...

/* transformer translation from the module intraprocedural transformer
 * to the module interprocedural transformer.
 *
 * Values related to variables local to the module must be
 * eliminated. Note that in C they should be eliminated earlier when
 * the effect of the local declaration is taken into account or when
 * the corresponding scope block is closed.
 */
...

/* Number of formal parameters in pl before a vararg is reached. The varargs
   are not analyzed. */
...

/* Recursive function to substitute formal and actual
 * array elements referenced or partially referenced by ar and fr.
 *
...
/* Substitute formal location entity l by a location entity
 * corresponding to ar, if it is possible.
 */
...
/* Compute the subscript list suffix in the abstract location
 * reference alr with respect to the actual reference ar and use this
 * subscript list to generate a new reference with fpv.
 *
 * This is used to translate preconditions from call site to callee.
 *
 * For instance, let s be a struct and a a field of s that is an
 * array. ar is s[a] and alr is s[a][3]. Then we try to substitute the
 * location entity al by the locatio nentity for reference fa[3],
 * where fa is the formal array associated to s[a].
 *
 * Caller:
 *
 * struct {int a[10];} s;
...
/* Early version. Does not deal with varargs like
 * any_user_call_site_to_transformer. Assume an equal number of formal
 ...
 * formal parameters passed by value, that is all but arrays, and
 * actual parameters.
 */
