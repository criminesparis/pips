/*
  ...
  $Id: identification.c 23495 2018-10-24 09:19:47Z coelho $
...
static void clean_statement_from_tags(string comment_portion, statement stat) 
{
  if (!statement_with_empty_comment_p(stat)) 
  {
    ...
    string searched_string = strdup(comment_portion);
    searched_string[strcspn(comment_portion, "%s")] = '\0';
    string comments = strdup(statement_comments(stat));
    char * next_line = strtok(comments, "\n");
