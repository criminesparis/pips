/*
  ...
  $Id: value.c 23412 2017-08-09 15:07:09Z irigoin $
...
#include "prettyprint.h" // type_to_full_string_definition
...
/* Does struct s contain directly or indirectly a field that may be
 * analyzable?
 */
...
/* Does array a contain directly or indirectly a field that may be
 * analyzable?
 */
...
/* FI: Nelson explains the motivation for
 * can_be_constant_path_p() but I do not understand them.
 *
 * He also explains the motivation for
 * strict_constant_path_p() and I do not understand them
 * either.
 *
 * Stricly speaking, this funtion should be called
 * analyzed_points_to_reference_p()
 */
...
/* This function could be made more robust by checking the storage of
   e. Formal parameters of analyzed type always have values. */
...
/* The address is the address of a variable or of a reference such as
 * &a[5], not a of a value. The reference must be a constant memory
 * access path. It does not have to be atomic, as struct are not
 * considered atomic, following Pierre Jouvelot's view point since
 ...
 *
 * We do not use array names as constant pointers because they alreay
 * are entities with types and storage. So the address of array a is
 * &a[0].
 *
 * References can be points-to references, i.e. fields can be encoded
 * as indices.
 */
...
/* For a given architecture, sizeof(t) is a constant. It has no old
 * value, no new value, it is a value.
 *
