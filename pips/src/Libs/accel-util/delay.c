/*
  ...
  $Id: delay.c 23644 2021-03-10 09:44:25Z coelho $
...
static bool recurse_statements_conflict_p(statement s, statement in) {
    ...
    gen_context_recurse(in, &c, statement_domain, gen_true2, do_recurse_statements_conflict_p);
    return c.conflict;
}
...
static context context_dup(context *c) {
  // FI: Missing initializer... I add statement_undefined...
  context cp = { c->result, gen_copy_seq(c->stats), c->backward, c->need_flatten , c->caller, statement_undefined };
  return cp;
}
...
static void promote_local_entities(statement s, entity caller, statement caller_statement) {
  pips_assert("true", caller==caller && caller_statement==caller_statement);
...
static void delay_communications_interprocedurally(context *c) {
    ...
    if(ENDP(callers)) {
        ...
        for(list citer=callers,siter=callers_statement;!ENDP(citer);POP(citer),POP(siter)) {
            ...
            gen_context_recurse(c->caller_statement,c,call_domain,gen_true2,do_delay_communications_interprocedurally);
...
static void do_remove_redundant_communications_in_sequence(sequence s, bool *need_flatten){
  pips_assert("true",need_flatten==need_flatten );
