/*
  ...
  $Id: pipsmake.c 23412 2017-08-09 15:07:09Z irigoin $
  ...
  const char* rname, const char* oname)
{
  ...
  */
  ...
  if (result == true)
  {
    list real_produced_resources =
      build_real_resources(oname, rule_produced(ru));
    bool res_found_p = false;

    pips_debug(5, "resource %s(%s) added to up_to_date with time stamp %d\n",
               rname, oname, db_time_of_resource(rname, oname));
    add_resource_to_make_cache(res_id);
    ...
    FOREACH(REAL_RESOURCE, rpr, real_produced_resources) {
      ...
      if (sres_id != res_id) {
        if (same_string_p(rname, srname)) {
          ...
          // Check that the sibling is currently obtained by the same rule,
          // because an activate might preempt it for some of the
          // produced resources?
    ...
    pips_assert("resource res is among the real resources produced by rule ru",
                res_found_p);
  ...
  else
  {
     ...
     *
     * FC 2016-11-16: ??? this is a bad idea, because this function we are in
     * is called by db_delete_obsolete_resources by a hook, and changing
     * the datastructures while scanning them leads to inconsistencies and
     * random aborts... see tspear-17.
     */
