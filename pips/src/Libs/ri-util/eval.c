 ...
 * evaluated regardless of the store and regardless of the target
 ...
 *
 * This function deals with all kinds of references, including the
 * points-to references which use fields as subscripts.
 *
 * There is an ambiguity for a[*] where * may be considered a constant
 * in some passes of PIPS. However, it denotes a constant set of
 * references and not a constant reference. The returned reference is
 * undefined.
 */
...
/* Allocate a new expression equivalent to e, but constant expressions
 * are evaluated.
 *
 * This function is used to normalize array dimensions to obtain type
 * equality for a[2] and a[4/2].
 */
