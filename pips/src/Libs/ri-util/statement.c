/*
  ...
  $Id: statement.c 23495 2018-10-24 09:19:47Z coelho $
 ...
 *
 * It is preserved because the current validation suite does not
 * include test cases covering its replacement.
 *
 ...
 * Can we really make an non empty_comment for block?
 *   cf text_statement_enclosed() or clean_up_sequences_rewrite()
 *      "block statement should be labelless, numberless and commentless"
 */
...
/** Build a statement sequence from a statement list
    ...
    @return the statement if there is only 1 statement in the list l or
...
/* Make a declaration(s) statement
   ...
   @param idl is a sorted list of variables that is copied and
   modified for insertion in the declaration. The order is
   preserved. No sharing is created between idl and s.

   @param sn is the statement number

   @param cs is the comment string associated to the declaration

   Note that additional information is dangerously hidden later as
   arguments to the call to CONTINUE.

   Note also that this function is closely linked to the C parser but
   is unfortunately used by transformations/registers.c in a much
   simpler form.
 */
...
/* Check that all statements contained in statement list sl are
...
/* The initialization expression list is integrated into the internal
 * representation as an argument list.
 *
 * For the time being, I am cheating because its a list of integer and
 ...
 * \param low       lower bound of the loop (an int expression)
 * \param up        upper bound of the loop (an int expression)
 * \param inc       increment step of the loop (an int expression)
 * \param b         body of the loop
 * \return          a loop statement with the condition low<i<up, with step inc and body b
 * see also make_new_loop_statement
 * Unlike make_new_loop_statement, make_loop_statement doesn't clone the body
 */
statement make_loop_statement(
    entity i,
    expression low,
    expression up,
    expression inc,
...
/**
 * Add a new declaration statement
 * (inspired by generic_add_declaration_statement)
 *
 * A new declaration statement is generated to declare e.
 * This statement is inserted before or after the
 * statements s in statement block_statement.
 * s must be a statement directly (no recursively) present
 * in statement block_statement
 *
 * Declarations are not only lists of entities at the block level and
 * in the symbol table, but also declaration statement to
 * carry the line number, comments, and the declarations local to the
 ...
 * maintain the consistency of the PIPS internal representation.
 *
 * \param block_statement   must be a sequence that contain statement s
 *                          strictly the same pointer than s and not a copy.
 *
 * \param s                 Statement after or before which declaration is inserted
 *
 * \param e                  Variable to declare.
 * The declaration of e at block_statement level is not checked.
 * The existence of a previous declaration statement for e is not checked either.
 * The caller is responsible for the consistency management between the
 * declaration statements in block_statement and the list of declaration
 * at block_statement level.
 *
 * \param before_p  declaration add before s if before_p==true
 *
 * \return          statement declaration generated or modified
 *                  statement_undefined if the add fail
 *
 * For the time being, a declaration statement is a continue
 * statement.
 * Beware that the possible dependencies between the new declaration
 * and existing declarations are not checked.
 * WARNING : block_statement is modify by side effect
 */
 ...
 *
 * NL: What is the difference with statement_in_statement_p?
 *     statement_in_statement_p certainly be better to keep, no global variable
 *     and use of gen_recurse_stop to stop the recursion
 */
 ...
 * \param s         the statement to work on
 * \return          list of pragma of the statement s, NIL if the statement doesn't have pragma
 */
...
/* Get the extension of a statement with pragma beginning with a prefix
   ...
   @param seed is the prefix a pragma has to begin with to be selected
...
/**
 * replace old_stat by new_stat in root_stat
 * this pass does not free old_stat
 * similar to replace_in_sequence_statement_with() in phrase/phrase_tools.c
 *   but without the constraint that old_stat is in a sequence
 * \param old_stat  statement to be replaced
 * \param new_stat  statement that will replace old_stat
 * \param root_stat statement where old_stat is searched to be replaced
 * \return          true if old_stat is found and replaced by new_stat
 *                  think to freed old_stat outside this function if the return is true
 */
