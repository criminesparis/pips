/*
  ...
  $Id: expression.c 23495 2018-10-24 09:19:47Z coelho $
 ...
 * NL : what the difference with expression_reference?
 */
 ...
 *
 * The validity of the input reference is not checked.
 * The validity of the output reference is not checked either.
 */
 ...
 *
 * This function copes with standard references, extended to pointer
 * references such as p[i], not with general points-to references,
 * which are extended to cope with structs and implicit
 * dereferencing. Points-to references are/should be handled in
 * effects-util, not in ri-util.
 */
 ...
 * Warning: This function certainly needs to be reviewed,
 *   maybe just testing the size of arguments (call), or the
 *   size of index (reference) may be sufficient as a first step.
 *   If their sizes are != 0, it may not return what we expect
 *   (for instance "-3" will return UNARY_MINUS_OPERATOR_NAME "--").
 *   But expression_to_entity is called by many functions and I worry
 *   about the side effects on them.
 */
...
/* Reuse expression e to build expression *(e) without cheking the
 * legality of the construct.
 */
...
*****************************************************************************/
expression size_of_actual_array(
  entity actual_array, list l_actual_ref, int i)
{
 ...
 *
 * If the array expression points to a multidimensional array, the
 * indices list should be as long as possible, compatible with the
 * array dimension.
 *
 *                    e (expression)
 *                    |
 ...
 *               sa    sil -> -> ->
 *             
 * The recursive normalization function works bottom up and, when
 * facing the pattern above, concatenate the il index list (or a
 * sublist) to the sil list. If the il list becomes empty, e is
 * connected directly to as and the pieces of data structure es, esub
 * and a are removed.
 *
 * gen_recurse() is used to apply the transformation on the way up in
 * expression trees. In the C parser, the transformation is applied to
 * a module statement and to the initialization expressions present in
 * the declarations.
 */
