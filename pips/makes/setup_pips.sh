#!/bin/bash
#
# $Id: setup_pips.sh 1262 2021-03-07 07:23:12Z coelho $
#
...
# where to get pips
SVN_CRI='https://scm.cri.mines-paristech.fr/svn'
...
optional arguments:
  ...
  --(dynamic|static)    enable/disable dynamic library compilation
  --pyconf PYCONF       use this python-config"
...
while [[ ${1:-} == -* ]]; do
    ...
    -h|--help)
        ...
        ;;
    -v|--version)
        echo 'version is $Id: setup_pips.sh 1262 2021-03-07 07:23:12Z coelho $'
        exit 0
        ;;
    -j[0-9]*)
        numjobs=${opt//-j/}
    ...
    -j|--jobs)
        ...
        shift
        ;;
    --python-config=*|--pyconf=*)
        pyconf=${opt#--*=}
        dyna=1
        ;;
    --python-config|--pyconf|--pc)
        pyconf="$1"
        dyna=1
        shift
        ;;
...
# devs are stricter:-)
# but they like their compilation to succeed:
# - 2020-05-29 no clobbered: issues in bison generated code with gcc 9.3
# - 2020-05-30 missing-field: issues with swig 4.0.1 & python 3.8
[ "$developer" ] && {
  echo "COPT += -Werror -Wno-error=clobbered -Wno-error=missing-field-initializers" >> "$config"
...
if [ "$dyna" ] ; then
  ...
  # note: will generate dynamic libraries if unavailable
fi

if [ "$pyconf" ] ; then
  echo "PYTHON_CONFIG = $pyconf" >> "$config"
  # use a consistent Python
  python=${pyconf%-config}
  echo "PYTHON = $python" >> "$config"
fi

type $python
py_version=$($python -c "import platform as p;print('.'.join(p.python_version_tuple()[0:2]))")
...
cat >> "$destination"/pipsrc.sh <<EOF
# fix ldpath, only useful when generating dynamic libraries
...
# fix pypath for pyps, only works with dynamic libraries
export PYTHONPATH="\$prod"/pips/lib/python/$py_version/pips:"\$PYTHONPATH"
