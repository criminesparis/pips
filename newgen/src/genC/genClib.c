/*
  ...
  $Id: genClib.c 1386 2018-10-24 09:18:26Z coelho $
...
{
  (void) gen_consistent_p(obj);
}

int gen_tabulated_consistent_p(int domain)
{
  cumulated_error_seen = 0;
  gen_mapc_tabulated(void_gen_consistent_p, domain);
...
struct gen_binding *bp ;
{
    ...
    {
	if (*bp->name=='s' && obj->s && !string_undefined_p(obj->s) &&
	    !allocated_memory_already_seen_p(obj->p))
...
struct multi_recurse
{
  ...
  gen_chunk            * previous;
  // current object, may be useful in a macro
  gen_chunk            * current;
};
			   ...
			   __attribute__((unused)) struct driver * dr)
{
  ...
  // set current object for call
  current_mrc->current = obj;
			    ...
			    __attribute__((unused)) struct driver * dr)
{
  ...
  // set current object before call
  current_mrc->current = obj;
...
/* return the current visited object, or NULL if not in a recursion.
   ...
   Ok, we should have it, but its name may not be known in a macro,
   or deep in a called function...
*/
gen_chunk * gen_get_current_object(void)
{
  return current_mrc? current_mrc->current: NULL;
}

/* Return current object of that type... or NULL if none found
 */
gen_chunk * gen_get_current_ancestor(int type)
{
  gen_chunk *current = gen_get_current_object();
  return current? gen_get_ancestor(type, current): NULL;
}
