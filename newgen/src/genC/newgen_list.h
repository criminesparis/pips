/*
  ...
  $Id: newgen_list.h 1386 2018-10-24 09:18:26Z coelho $
...
extern void *gen_chunk_identity(gen_chunk);
...
extern void gen_mapc_tabulated(void (*)(gen_chunk*), int);
...
extern void gen_closure(gen_closure_func_t, const list);
...
extern list gen_VOID_STAR_cons(const void *, const list);
...
extern bool gen_list_cyclic_p (const list);
extern list gen_list_head(list *, int);
extern list gen_common_prefix(const list, const list);

extern void gen_substitute_chunk_by_list(list * pl, const void * o, list sl);

/* sweep a list...
 */
typedef enum { gen_sweep_forward, gen_sweep_backward } gen_sweep_direction ;
typedef struct __gen_sweep_state * gen_sweep_state;
