# $Id: main.mk 1263 2023-07-10 11:46:43Z coelho $
#
...
# force dynamic libraries for pyps
ifdef PIPS_ENABLE_PYPS
  ifndef WITH_DYNAMIC_LIBRARIES
...
# idem, but another way (no versions)
ifdef ALSO_WITH_DYNAMIC_LIBRARIES
CFLAGS	+= -fPIC
endif
...
# work around flex 2.6.0 issues (yy_size_t) changed to (int)...
# which breaks gcc -Werror
SCANFIX = sed -e '/^FILE *\*/s,=[^,;]*,,g' \
      -e 's/\(([a-z_]*)\)\( ((yy_n_chars) + number_to_move) > \)\(YY_CURRENT_BUFFER_LVALUE->yy_buf_size\)/\1\2\1\3/'
...
# simple target to help debug cpp-related issues
$(ARCH)/%.i: %.c; $(COMPILE) -E `pwd`/$< -o $@
...
ifndef INC_CFILES
INC_CFILES	= $(LIB_CFILES) $(INC_YFILES)
endif # INC_CFILES

name	= $(subst -,_, $(notdir $(CURDIR)))

$(INC_TARGET).tmp: $(DERIVED_CFILES)
	# use a temporary header file
	$(COPY) $(TARGET)-local.h $(INC_TARGET);
	# then generate actual header file
	{ \
	  ...
	  echo "/* $(name)-local.h */"; \
	  cat $(TARGET)-local.h; \
	  echo "/* cproto-generated files */"; \
	  $(PROTOIZE) $(INC_CFILES) $(DERIVED_CFILES) | \
...
# FC: testing a simple version
ifdef ALSO_WITH_DYNAMIC_LIBRARIES

$(ARCH)/$(LIB_TARGET:%.a=%.so): $(LIB_OBJECTS)
	$(LD) -o $@ -shared $(LDFLAGS) $(LDOPT) $(LIB_OBJECTS) $(DYNLDFLAGS)

INSTALL_LIB	+=   $(LIB_TARGET:%.a=%.so)

endif # LIB_TARGET_DYNAMIC
...
phase5: runtime-install
install: runtime-install

.PHONY: runtime-install
runtime-install: .build_runtime

.build_runtime:
	$(INSTALL) -d $(RTM.d)
	$(INSTALL) -m 644 $(INSTALL_RTM) $(RTM.d)
	touch $@

unbuild: runtime-unbuild
runtime-unbuild:
